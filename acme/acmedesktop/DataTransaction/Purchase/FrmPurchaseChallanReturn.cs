...
{
    public partial class FrmPurchaseChallanReturn : Form
    {
        ...
        string _ChallanNo = "";
        IPurchaseChallanReturn _objPurchaseChallanReturn = new ClsPurchaseChallanReturn(); 
        ...
        public FrmPurchaseChallanReturn()
        {
            InitializeComponent();
            ...
            _IsSaveAndPrint = _objDocPrintSetting.CheckIsSaveAndPrint("PCR");

        }

        private void FrmPurchaseChallanReturn_Load(object sender, EventArgs e)
        {
        ...
        private void FrmPurchaseChallanReturn_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void TxtGridQty_Validating(object sender, System.ComponentModel.CancelEventArgs e)
        {
            if (!string.IsNullOrEmpty(TxtGridParticular.Text))
            {
                ...
                {
                    if (Convert.ToInt32(_objPurchaseInvoice.GetPurchaseQuantityProductWise(TxtRefBillNo.Text, Convert.ToInt32(Grid.CurrentRow.Cells["ProductId"].Value), Convert.ToInt32(Grid.CurrentRow.Cells["Sno"].Value))) < Convert.ToInt32(_Qty))
                    {
                        MessageBox.Show("Return quantity can not be greater than purchase quantity.", "Mr. Solutions", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        TxtGridQty.Focus();
                        return;
                    }
                }
        ...
        private void BtnDelete_Click(object sender, EventArgs e)
        {
            ...
            Text = "Purchase Challan Return [DELETE]";
        ...
        {
            DocPrintingOption frm = new DocPrintingOption("PCR", "Purchase Challan Return", "");
            frm.ShowDialog();
            frm.Dispose();
        }
        ...
        private void BtnVoucherNoSearch_Click(object sender, EventArgs e)
        {
            ...
            PickList frmPickList = new PickList("PurchaseChallanReturnVoucher", _SearchKey);
            if (PickList.dt.Rows.Count > 0)
            {
                ...
                if (frmPickList.SelectedList.Count > 0 && _Tag != "NEW")
                {
                    ...
                    DataSet ds = _objPurchaseChallanReturn.GetDataPurchaseChallanReturnVoucher(_VoucherNo);
                    ClearFld();
                    SetData(ds);
                }
                frmPickList.Dispose();
        ...
        private void TxtVoucherNo_Validating(object sender, System.ComponentModel.CancelEventArgs e)
        {
			...
			if (_Tag == "NEW")
			{
				if (!string.IsNullOrEmpty(_objPurchaseChallanReturn.IsExistsVNumber(TxtVoucherNo.Text)))
				{
			...
			else
			{
				if (this._VoucherNo != TxtVoucherNo.Text.Trim())
				{
					...
					DataSet ds = _objPurchaseChallanReturn.GetDataPurchaseChallanVoucherDataForReturn(this._VoucherNo);
					ClearFld();
					SetData(ds);
				}
			}
        ...
        {
            FrmUDFMasterEntry frm = new FrmUDFMasterEntry("Purchase Master Global", _VoucherNo, "PCR");
            frm.ShowDialog();
        ...
        private void BtnOk_Click(object sender, EventArgs e)
        {
            ...
            if (_NetAmt < 0)
            {
                MessageBox.Show("Purchase Challan Return invoice negative should not be save.", "Mr Solution", MessageBoxButtons.OK, MessageBoxIcon.Information);
                BtnOk.Focus();
                return;
            }
            ...
            _objPurchaseChallanReturn.Model.EntryFromProject = "Normal";
            _objPurchaseChallanReturn.Model.Gadget = "Desktop";
            _objPurchaseChallanReturn.Model.ReferenceNo = TxtRefBillNo.Text;
            _objPurchaseChallanReturn.Model.ReferenceDate = Refdate;
            _objPurchaseChallanReturn.Model.ReferenceMiti = Refmiti;

            _objPurchaseChallanReturn.Model.PartyName = TxtVendor.Text;
            _objPurchaseChallanReturn.Model.ChequeDate = null;
            _objPurchaseChallanReturn.Model.UdfDetails = ClsGlobal.UDFExistingDataTableDetails;
            _objPurchaseChallanReturn.Model.UdfMaster = ClsGlobal.UDFExistingDataMaster;
            _objPurchaseChallanReturn.Model.VDate = Convert.ToDateTime(TxtDate.Text);
            _objPurchaseChallanReturn.Model.Tag = _Tag;

            if (_Tag == "NEW")
            {
                string[] VoucherNoDetails = _objCommon.GetVoucherNo(TxtVoucherNo.Tag.ToString(), "Purchase Challan Return", ClsGlobal.BranchId, ClsGlobal.CompanyUnitId);
                _objPurchaseChallanReturn.Model.DocId = ((TxtVoucherNo.Tag.ToString() == "") ? 0 : Convert.ToInt32(TxtVoucherNo.Tag.ToString()));
                _objPurchaseChallanReturn.Model.VoucherNo = (!string.IsNullOrEmpty(VoucherNoDetails[0])) ? VoucherNoDetails[0] : TxtVoucherNo.Text.Trim();
            }
            else
                _objPurchaseChallanReturn.Model.VoucherNo = TxtVoucherNo.Text.Trim();

            _objPurchaseChallanReturn.Model.VTime = _objDate.GetServerDateTime();
            _objPurchaseChallanReturn.Model.VMiti = TxtMiti.Text;
            _objPurchaseChallanReturn.Model.LedgerId = Convert.ToInt32(TxtVendor.Tag.ToString());


            _objPurchaseChallanReturn.Model.SalesmanId = Convert.ToInt32(TxtSalesman.Tag.ToString());

            if (!string.IsNullOrEmpty(TxtDepartment.Text))
            {
                string[] dept = TxtDepartment.Tag.ToString().Split('|');
                _objPurchaseChallanReturn.Model.DepartmentId1 = ((dept[0].ToString() == "") ? 0 : Convert.ToInt32(dept[0].ToString()));
                _objPurchaseChallanReturn.Model.DepartmentId2 = ((dept[1].ToString() == "") ? 0 : Convert.ToInt32(dept[1].ToString()));
                _objPurchaseChallanReturn.Model.DepartmentId3 = ((dept[2].ToString() == "") ? 0 : Convert.ToInt32(dept[2].ToString()));
                _objPurchaseChallanReturn.Model.DepartmentId4 = ((dept[3].ToString() == "") ? 0 : Convert.ToInt32(dept[3].ToString()));
            }
            _objPurchaseChallanReturn.Model.BranchId = ClsGlobal.BranchId;
            _objPurchaseChallanReturn.Model.CompanyUnitId = ClsGlobal.CompanyUnitId;
            if (!string.IsNullOrEmpty(TxtCurrency.Tag.ToString()))
            {

                _objPurchaseChallanReturn.Model.CurrencyId = Convert.ToInt32(TxtCurrency.Tag.ToString());
            }
            _objPurchaseChallanReturn.Model.CurrencyRate = _CurrencyRate;
            decimal.TryParse(LblBasicAmt.Text.Trim(), out decimal BasicAmt);
            _objPurchaseChallanReturn.Model.BasicAmount = BasicAmt;
            decimal.TryParse(TxtBillTermAmt.Text.Trim(), out decimal TermAmt);
            _objPurchaseChallanReturn.Model.TermAmount = TermAmt;
            decimal.TryParse(LblNetAmt.Text.Trim(), out decimal NetAmt);
            _objPurchaseChallanReturn.Model.NetAmount = NetAmt;
            decimal.TryParse(LblNetAmt.Text.Trim(), out decimal localNetAmount);
            _objPurchaseChallanReturn.Model.LocalNetAmount = localNetAmount;
            _objPurchaseChallanReturn.Model.SubLedgerId = Convert.ToInt32(TxtSubledger.Tag.ToString());


            if (!string.IsNullOrEmpty(TxtBillingAddress.Text.ToString().Trim()))
            {
                _objPurchaseChallanReturn.ModelBillAddress.LedgerId = Convert.ToInt32(TxtVendor.Tag.ToString());
                _objPurchaseChallanReturn.ModelBillAddress.BillingAddress = TxtBillingAddress.Text.Trim();
                _objPurchaseChallanReturn.ModelBillAddress.BillingCity = TxtBillingCity.Text.Trim();
                _objPurchaseChallanReturn.ModelBillAddress.BillingState = TxtBillingState.Text.Trim();
                _objPurchaseChallanReturn.ModelBillAddress.BillingCountry = TxtBillingCountry.Text.Trim();
                _objPurchaseChallanReturn.ModelBillAddress.BillingEmail = TxtBillingEmail.Text.Trim();
                _objPurchaseChallanReturn.ModelBillAddress.ShippingAddress = TxtShippingAddress.Text.Trim();
                _objPurchaseChallanReturn.ModelBillAddress.ShippingCity = TxtShippingCity.Text.Trim();
                _objPurchaseChallanReturn.ModelBillAddress.ShippingState = TxtShippingState.Text.Trim();
                _objPurchaseChallanReturn.ModelBillAddress.ShippingCountry = TxtShippingCountry.Text.Trim();
                _objPurchaseChallanReturn.ModelBillAddress.ShippingEmail = TxtShippingEmail.Text.Trim();
            }
            _objPurchaseChallanReturn.Model.PartyName = !string.IsNullOrEmpty(CashPartyViewModel.PartyName) ? CashPartyViewModel.PartyName : TxtVendor.Text.Trim();
            _objPurchaseChallanReturn.Model.PartyAddress = CashPartyViewModel.PartyAddress;
            _objPurchaseChallanReturn.Model.PartyVatNo = CashPartyViewModel.PartyVatNo;
            if (!string.IsNullOrEmpty(CashPartyViewModel.ChequeNo))
            {
                _objPurchaseChallanReturn.Model.ChequeNo = CashPartyViewModel.ChequeNo;
                _objPurchaseChallanReturn.Model.ChequeDate = Convert.ToDateTime(CashPartyViewModel.ChequeDate);
                _objPurchaseChallanReturn.Model.ChequeMiti = null;
            }
            else
            {
                _objPurchaseChallanReturn.Model.ChequeNo = null;
                _objPurchaseChallanReturn.Model.ChequeDate = null;
                _objPurchaseChallanReturn.Model.ChequeMiti = null;
            }
            _objPurchaseChallanReturn.Model.PartyMobileNo = CashPartyViewModel.PartyMobileNo;
            _objPurchaseChallanReturn.Model.PartyEmail = CashPartyViewModel.PartyEmail;

            _objPurchaseChallanReturn.ModelOtherDetails.Transport = TxtTransport.Text.Trim();
            _objPurchaseChallanReturn.ModelOtherDetails.VehicleNo = TxtVehicleNo.Text.Trim();
            _objPurchaseChallanReturn.ModelOtherDetails.Package = TxtPackage.Text.Trim();
            _objPurchaseChallanReturn.ModelOtherDetails.CnNo = TxtCnNo.Text.Trim();

            if (TxtCnDate.Text != "  /  /")
            {
                if (ClsGlobal.DateType == "M")
                    _objPurchaseChallanReturn.ModelOtherDetails.CnDate = _objDate.GetDate(TxtCnDate.Text);
                else
                    _objPurchaseChallanReturn.ModelOtherDetails.CnDate = Convert.ToDateTime(TxtCnDate.Text);
            }
            else
                _objPurchaseChallanReturn.ModelOtherDetails.CnDate = null;

            _objPurchaseChallanReturn.ModelOtherDetails.CnFreight = TxtCnFreight.Text.Trim();
            _objPurchaseChallanReturn.ModelOtherDetails.Advance = TxtAdvance.Text.Trim();
            _objPurchaseChallanReturn.ModelOtherDetails.BalFreight = TxtBalFreight.Text.Trim();

            if (CmbCnType.Text == "Due")
            {
                _objPurchaseChallanReturn.ModelOtherDetails.CnType = "D";
            }
            else if (CmbCnType.Text == "To Pay")
            {
                _objPurchaseChallanReturn.ModelOtherDetails.CnType = "T";
            }
            else if (CmbCnType.Text == "Paid")
            {
                _objPurchaseChallanReturn.ModelOtherDetails.CnType = "P";
            }
            else if (CmbCnType.Text == "Free")
            {
                _objPurchaseChallanReturn.ModelOtherDetails.CnType = "F";
            }
            else if (CmbCnType.Text == "")
            {
                _objPurchaseChallanReturn.ModelOtherDetails.CnType = null;
            }
            _objPurchaseChallanReturn.ModelOtherDetails.DriverName = TxtDriverName.Text.Trim();
            _objPurchaseChallanReturn.ModelOtherDetails.DriverLicNo = TxtDriverLicenseNo.Text.Trim();
            _objPurchaseChallanReturn.ModelOtherDetails.DriverMobileNo = TxtDriverMobileNo.Text.Trim();
            _objPurchaseChallanReturn.ModelOtherDetails.ContractNo = TxtContactNo.Text.Trim();
            if (TxtContactDate.Text != "  /  /")
            {
                if (ClsGlobal.DateType == "M")
                    _objPurchaseChallanReturn.ModelOtherDetails.ContractDate = _objDate.GetDate(TxtContactDate.Text);
                else
                    _objPurchaseChallanReturn.ModelOtherDetails.ContractDate = Convert.ToDateTime(TxtContactDate.Text);
            }
            else
                _objPurchaseChallanReturn.ModelOtherDetails.ContractDate = null;

            _objPurchaseChallanReturn.ModelOtherDetails.ExpInvNo = TxtExportInvNo.Text.Trim();

            if (TxtExportInvDate.Text != "  /  /")
            {
                if (ClsGlobal.DateType == "M")
                    _objPurchaseChallanReturn.ModelOtherDetails.ExpInvDate = _objDate.GetDate(TxtExportInvDate.Text);
                else
                    _objPurchaseChallanReturn.ModelOtherDetails.ExpInvDate = Convert.ToDateTime(TxtExportInvDate.Text);
            }
            else
                _objPurchaseChallanReturn.ModelOtherDetails.ExpInvDate = null;

            _objPurchaseChallanReturn.ModelOtherDetails.PoNo = TxtPONo.Text.Trim();

            if (TxtPODate.Text != "  /  /")
            {
                if (ClsGlobal.DateType == "M")
                    _objPurchaseChallanReturn.ModelOtherDetails.PoDate = _objDate.GetDate(TxtPODate.Text);
                else
                    _objPurchaseChallanReturn.ModelOtherDetails.PoDate = Convert.ToDateTime(TxtPODate.Text);
            }
            else
                _objPurchaseChallanReturn.ModelOtherDetails.PoDate = null;

            _objPurchaseChallanReturn.ModelOtherDetails.DocBank = TxtDocBank.Text.Trim();
            _objPurchaseChallanReturn.ModelOtherDetails.LcNo = TxtLCNo.Text.Trim();
            _objPurchaseChallanReturn.ModelOtherDetails.CustomName = TxtCustomerName.Text.Trim();
            _objPurchaseChallanReturn.ModelOtherDetails.Cofd = TxtCoFd.Text.Trim();

            PurchaseChallanReturnDetailsViewModel _PurchaseChallanReturnDetails = null;
            ...
            foreach (DataGridViewRow ro in Grid.Rows)
            {
                ...
                {
                    _PurchaseChallanReturnDetails = new PurchaseChallanReturnDetailsViewModel();
                    _PurchaseChallanReturnDetails.VoucherNo = _objPurchaseChallanReturn.Model.VoucherNo;
                    _PurchaseChallanReturnDetails.Sno = Grid.Rows.IndexOf(ro) + 1;
                    _PurchaseChallanReturnDetails.ProductId = Convert.ToInt32(ro.Cells["ProductId"].Value.ToString());

                    if (ro.Cells["GodownId"].Value != null)
                        _PurchaseChallanReturnDetails.GodownId = string.IsNullOrEmpty(ro.Cells["GodownId"].Value.ToString()) ? 0 : Convert.ToInt32(ro.Cells["GodownId"].Value.ToString());
                    else
                        _PurchaseChallanReturnDetails.GodownId = 0;

                    if (ro.Cells["AltQty"].Value != null)
                    {
                        decimal.TryParse(ro.Cells["AltQty"].Value.ToString().Trim(), out decimal _AltQty);
                        _PurchaseChallanReturnDetails.AltQty = _AltQty;
                    }
                    else
                        _PurchaseChallanReturnDetails.AltQty = 0;
                    if (ro.Cells["AltQty"].Value != null)
                    {
                        _PurchaseChallanReturnDetails.ProductAltUnit = !string.IsNullOrEmpty(ro.Cells["ProductAltUnitId"].Value.ToString()) ? Convert.ToInt32(ro.Cells["ProductAltUnitId"].Value.ToString()) : 0;
                    }

                    _PurchaseChallanReturnDetails.Qty = ro.Cells["Qty"].Value != null ? Convert.ToDecimal(ro.Cells["Qty"].Value.ToString().Trim()) : 0;

                    if (ro.Cells["ProductUnitId"].Value != null)
                        _PurchaseChallanReturnDetails.ProductUnit = string.IsNullOrEmpty(ro.Cells["ProductUnitId"].Value.ToString()) ? 0 : Convert.ToInt32(ro.Cells["ProductUnitId"].Value.ToString());
                    else
                        _PurchaseChallanReturnDetails.ProductUnit = 0;
                    _PurchaseChallanReturnDetails.ConversionRatio = ro.Cells["QtyConversion"].Value != null ? Convert.ToDecimal(ro.Cells["QtyConversion"].Value.ToString().Trim()) : 0;
                    _PurchaseChallanReturnDetails.PurchaseRate = ro.Cells["Rate"].Value != null ? Convert.ToDecimal(ro.Cells["Rate"].Value.ToString().Trim()) : 0;
                    _PurchaseChallanReturnDetails.BasicAmount = ro.Cells["BasicAmt"].Value != null ? Convert.ToDecimal(ro.Cells["BasicAmt"].Value.ToString().Trim()) : 0;
                    _PurchaseChallanReturnDetails.TermAmount = ro.Cells["TermsAmt"].Value != null ? Convert.ToDecimal(ro.Cells["TermsAmt"].Value.ToString().Trim()) : 0;
                    _PurchaseChallanReturnDetails.NetAmount = ro.Cells["NetAmt"].Value != null ? Convert.ToDecimal(ro.Cells["NetAmt"].Value.ToString().Trim()) : 0;
                    if (ro.Cells["OrderSNo"].Value != null)
                        _PurchaseChallanReturnDetails.OrderSNo = ro.Cells["OrderSNo"].Value != null ? Convert.ToInt32(ro.Cells["OrderSNo"].Value.ToString().Trim()) : 0;
                    else
                        _PurchaseChallanReturnDetails.OrderSNo = 0;
                    _PurchaseChallanReturnDetails.OrderNo = ro.Cells["OrderNo"].Value?.ToString().Trim();
                    if (ro.Cells["ChallanInvoiceSNo"].Value != null)
                        _PurchaseChallanReturnDetails.ChallanInvoiceSNo = ro.Cells["ChallanInvoiceSNo"].Value != null ? Convert.ToInt32(ro.Cells["ChallanInvoiceSNo"].Value.ToString().Trim()) : 0;
                    else
                        _PurchaseChallanReturnDetails.ChallanInvoiceSNo = 0;
                    _PurchaseChallanReturnDetails.ChallanInvoiceNo = ro.Cells["ChallanInvoiceNo"].Value?.ToString().Trim();
                    ...
                    _PurchaseChallanReturnDetails.AdditionalDesc = ro.Cells["AdditionalDesc"].Value != null ? ro.Cells["AdditionalDesc"].Value.ToString() : ro.Cells["Particular"].Value.ToString().Trim();
                    _PurchaseChallanReturnDetails.FreeQty = 0;
                    _objPurchaseChallanReturn.ModelDetails.Add(_PurchaseChallanReturnDetails);
                    ...
                    if (ro.Cells["TermDetails"].Value != null)
                    {
                        if (!string.IsNullOrEmpty(ro.Cells["TermDetails"].Value.ToString()))
                        {
                            ...
                            for (int i = 0; i < termCodeArr.Count(); i++)
                            {
                                ...
                                _objPurchaseChallanReturn.ModelTerms.Add(_PurchaseInvoiceTerm);
                            }
                        }
                    }
                }
            ...
            if (!string.IsNullOrEmpty(TxtBillTermAmt.Tag.ToString().Trim()))
            {
                ...
                if (val2.Length > 1)
                {
                    ...
                    {
                        _PurchaseInvoiceTerm = new TermViewModel();
                        _PurchaseInvoiceTerm.VoucherNo = _objPurchaseChallanReturn.Model.VoucherNo;
            ...
            _objPurchaseChallanReturn.Model.DocId = Convert.ToInt32(TxtVoucherNo.Tag.ToString());
            string output = _objPurchaseChallanReturn.SavePurchaseChallanReturn();
        ...
        {
            Common.PickList frmPickList = new Common.PickList("PurchaseChallanForChallanReturn", _SearchKey);
            if (Common.PickList.dt.Rows.Count > 0)
            {
                frmPickList.ShowDialog();
                if (frmPickList.SelectedList.Count > 0)
        ...
        private void TxtRefBillNo_Validating(object sender, System.ComponentModel.CancelEventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(TxtRefBillNo.Text.Trim()))
            {
                if (this._Tag == "NEW")
                {
                    DataSet ds = _objPurchaseChallanReturn.GetDataPurchaseChallanVoucherDataForReturn(TxtRefBillNo.Text.Trim());
                    Grid.Rows.Clear();
                    Grid.Rows.Add();
                    Grid.Enabled = true;
                    SetRefBillData(ds);
        ...
        private void BtnNew_Click(object sender, EventArgs e)
        {
            ClearFld();
            _Tag = "NEW";
            Text = "Purchase Challan Return [NEW]";
            ...
            {
                Utility.GetVoucherNo1("Purchase Challan Return", TxtVoucherNo, TxtDate, _Tag, "");
            }
            else if (TxtRefBillNo.Enabled == true)
            {
                Utility.GetVoucherNo2("Purchase Challan Return", TxtVoucherNo, TxtRefBillNo, _Tag, "");
            }
            else
            {
                Utility.GetVoucherNo1("Purchase Challan Return", TxtVoucherNo, TxtDate, _Tag, "");
        ...
        private void BtnEdit_Click(object sender, EventArgs e)
        {
            _Tag = "EDIT";
            ControlEnableDisable(false, true);
            Text = "Purchase Challan Return [EDIT]";
        }
        ...
        private void SetData(DataSet ds)
        {
            ...
            foreach (DataRow drDetails in dtDetails.Rows)
            {
                ...
                Grid.Rows[Grid.Rows.Count - 1].Cells["ChallanInvoiceNo"].Value = TxtRefBillNo.Text;
                Grid.Rows[Grid.Rows.Count - 1].Cells["ChallanInvoiceSNo"].Value = Grid.Rows.Count.ToString();
