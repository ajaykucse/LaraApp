...
namespace acmedesktop.DataTransaction
{
    public partial class FrmTerm : Form
    {
		...
		private bool SetTextBoxValueToGrid()
		{
			...
			ro.Cells["TermRate"].Value = string.IsNullOrEmpty(TxtGridRatePercent.Text.Trim()) ? "0" : TxtGridRatePercent.Text.Trim();
			ro.Cells["Amount"].Value = string.IsNullOrEmpty(TxtGridAmount.Text.Trim()) ? "0" : TxtGridAmount.Text.Trim();
			return true;
		}
        ...
        public decimal TermAmount()
		{
            ...
            if (dt.Rows.Count > 0)
            {
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    else if (row["Basis"].ToString().Trim().ToUpper() == "Q")
                    {
                        if (TermRate > 0)
                        {
