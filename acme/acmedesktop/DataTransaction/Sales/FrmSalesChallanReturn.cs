...
{
    public partial class FrmSalesChallanReturn : Form
    {
        ...
        ISalesChallanReturn _objSalesChallanReturn = new ClsSalesChallanReturn();
        ...
        public FrmSalesChallanReturn()
        {
            ...
            _IsSaveAndPrint = _objDocPrintSetting.CheckIsSaveAndPrint("SCR");
        }
        ...
        private void BtnEdit_Click(object sender, EventArgs e)
        {
            ...
            Text = "Sales Challan Return [EDIT]";
        }
        ...
        private void BtnDelete_Click(object sender, EventArgs e)
        {
            ...
            Text = "Sales Challan Return [DELETE]";
        ...
        private void BtnNew_Click(object sender, EventArgs e)
        {
            ...
            {
                Utility.GetVoucherNo1("Sales Challan Return", TxtVoucherNo, TxtDate, _Tag, "", _DocId);
            }
            else if (TxtRefBillNo.Enabled == true)
            {
                Utility.GetVoucherNo2("Sales Challan Return", TxtVoucherNo, TxtRefBillNo, _Tag, "", _DocId);
            }
            else
            {
                Utility.GetVoucherNo2("Sales Challan Return", TxtVoucherNo, TxtCustomer, _Tag, "", _DocId);
            }
        }
        ...
        private void FrmSalesChallanReturn_Load(object sender, EventArgs e)
        {
        ...
        {
            DocPrintingOption frm = new DocPrintingOption("SCR", "Sales Challan Return", "");
            frm.ShowDialog();
        }
        ...
        {
            PickList frmPickList = new PickList("SalesChallanReturnVoucher", _SearchKey);
            if (PickList.dt.Rows.Count > 0)
            {
                ...
                if (frmPickList.SelectedList.Count > 0 && _Tag != "NEW")
                {
                    ...
                    DataSet ds = _objSalesChallanReturn.GetDataSalesChallanReturnVoucher(_VoucherNo);
                    ClearFld();
                    SetData(ds);
                }
                frmPickList.Dispose();
            ...
            {
                MessageBox.Show("No list available in Sales Challan Return voucher.", "Mr. Solution", MessageBoxButtons.OK, MessageBoxIcon.Information);
                TxtVoucherNo.Focus();
                return;
            }
            TxtVoucherNo.Focus();
        ...
        private void TxtVoucherNo_Validating(object sender, CancelEventArgs e)
        {
			...
			if (_Tag == "NEW")
			{
				if (!string.IsNullOrEmpty(_objSalesChallanReturn.IsExistsVNumber(TxtVoucherNo.Text)))
				{
			...
			else
			{
				if (this._VoucherNo != TxtVoucherNo.Text.Trim())
				{
					...
					DataSet ds = _objSalesChallanReturn.GetDataSalesChallanReturnVoucher(this._VoucherNo);
					ClearFld();
					SetData(ds);
				}
			}
        ...
        private void FrmSalesChallanReturn_KeyDown(object sender, KeyEventArgs e)
        {
		...
		{
			Common.PickList frmPickList = new Common.PickList("SalesChallanVoucherForChallanReturn", _SearchKey);
			if (Common.PickList.dt.Rows.Count > 0)
			{
				frmPickList.ShowDialog();
				if (frmPickList.SelectedList.Count > 0)
		...
		private void TxtRefBillNo_Validating(object sender, CancelEventArgs e)
		{
			...
			if (!string.IsNullOrEmpty(TxtRefBillNo.Text.Trim()))
			{
				if (this._Tag == "NEW")
				{
					DataSet ds = _objSalesChallanReturn.GetDataSalesChallanVoucherDataForReturn(TxtRefBillNo.Text.Trim());
					Grid.Rows.Clear();
					Grid.Rows.Add();
					Grid.Enabled = true;
					SetRefBillData(ds);
		...
		private void SetRefBillData(DataSet ds)
		{
			...
			foreach (DataRow drDetails in dtDetails.Rows)
			{
				...
				Grid.Rows[Grid.Rows.Count - 1].Cells["AltStockQty"].Value = ClsGlobal.DecimalFormate(Convert.ToDecimal(drDetails["AltQty"].ToString()), 1, ClsGlobal._AltQtyDecimalFormat).ToString();
				Grid.Rows[Grid.Rows.Count - 1].Cells["StockQty"].Value = ClsGlobal.DecimalFormate(Convert.ToDecimal(drDetails["Qty"].ToString()), 1, ClsGlobal._QtyDecimalFormat).ToString();
                Grid.Rows[Grid.Rows.Count - 1].Cells["ChallanNo"].Value = TxtRefBillNo.Text;
                Grid.Rows[Grid.Rows.Count - 1].Cells["ChallanSNo"].Value = Grid.Rows.Count.ToString();
                Grid.Rows[Grid.Rows.Count - 1].Cells["OrderNo"].Value = drDetails["OrderNo"].ToString();
                if (string.IsNullOrEmpty(drDetails["OrderSNo"].ToString()))
                    Grid.Rows[Grid.Rows.Count - 1].Cells["OrderSNo"].Value = null;
                else
                    Grid.Rows[Grid.Rows.Count - 1].Cells["OrderSNo"].Value = drDetails["OrderSNo"].ToString();
                Grid.Rows[Grid.Rows.Count - 1].Cells["DispatchOrderNo"].Value = drDetails["DispatchOrderNo"].ToString();
                if (string.IsNullOrEmpty(drDetails["DispatchOrderSNo"].ToString()))
                    Grid.Rows[Grid.Rows.Count - 1].Cells["DispatchOrderSNo"].Value = null;
                else
                    Grid.Rows[Grid.Rows.Count - 1].Cells["DispatchOrderSNo"].Value = drDetails["DispatchOrderSNo"].ToString();
                Grid.Rows[Grid.Rows.Count - 1].Cells["QuotationNo"].Value = drDetails["QuotationNo"].ToString();
                if (string.IsNullOrEmpty(drDetails["QuotationSNo"].ToString()))
                    Grid.Rows[Grid.Rows.Count - 1].Cells["QuotationSNo"].Value = null;
                else
		...
		{			
			FrmUDFMasterEntry frm = new FrmUDFMasterEntry("Sales Master Global", _VoucherNo, "SCR");
			frm.ShowDialog();
		...
		private void BtnOk_Click(object sender, EventArgs e)
		{
			...
			if (_NetAmt < 0)
			{
				MessageBox.Show("Sales Challan Return negative should not be save.", "Mr Solution", MessageBoxButtons.OK, MessageBoxIcon.Information);
				BtnOk.Focus();
				return;
			}
			...
			_objSalesChallanReturn.Model.EntryFromProject = "Normal";
			_objSalesChallanReturn.Model.Gadget = "Desktop";
			_objSalesChallanReturn.Model.EnterBy = ClsGlobal.LoginUserCode;
			_objSalesChallanReturn.Model.ReferenceNo = TxtRefBillNo.Text;
			_objSalesChallanReturn.Model.ReferenceDate = Refdate;
			_objSalesChallanReturn.Model.ReferenceMiti = Refmiti;

			_objSalesChallanReturn.Model.PartyName = !string.IsNullOrEmpty(CashPartyViewModel.PartyName) ? CashPartyViewModel.PartyName : TxtCustomer.Text.Trim();
			_objSalesChallanReturn.Model.PartyAddress = CashPartyViewModel.PartyAddress;
			_objSalesChallanReturn.Model.PartyVatNo = CashPartyViewModel.PartyVatNo;
			if (!string.IsNullOrEmpty(CashPartyViewModel.ChequeNo))
			{
				_objSalesChallanReturn.Model.ChequeNo = CashPartyViewModel.ChequeNo;
				_objSalesChallanReturn.Model.ChequeDate = Convert.ToDateTime(CashPartyViewModel.PartyVatNo);
				_objSalesChallanReturn.Model.ChequeMiti = null;
			}
			else
			{
				_objSalesChallanReturn.Model.ChequeNo = null;
				_objSalesChallanReturn.Model.ChequeDate = null;
				_objSalesChallanReturn.Model.ChequeMiti = null;
			}
			_objSalesChallanReturn.Model.PartyMobileNo = CashPartyViewModel.PartyMobileNo;
			_objSalesChallanReturn.Model.PartyEmail = CashPartyViewModel.PartyEmail;
			
			_objSalesChallanReturn.Model.UdfDetails = ClsGlobal.UDFExistingDataTableDetails;
			_objSalesChallanReturn.Model.UdfMaster = ClsGlobal.UDFExistingDataMaster;
			_objSalesChallanReturn.Model.VDate = Convert.ToDateTime(TxtDate.Text);
			_objSalesChallanReturn.Model.Tag = _Tag;

			if (_Tag == "NEW")
			{
				string[] VoucherNoDetails = _objCommon.GetVoucherNo(TxtVoucherNo.Tag.ToString(), "Sales Challan Return", ClsGlobal.BranchId, ClsGlobal.CompanyUnitId);
				_objSalesChallanReturn.Model.DocId = ((TxtVoucherNo.Tag.ToString() == "") ? 0 : Convert.ToInt32(TxtVoucherNo.Tag.ToString()));
				_objSalesChallanReturn.Model.VoucherNo = (!string.IsNullOrEmpty(VoucherNoDetails[0])) ? VoucherNoDetails[0] : TxtVoucherNo.Text.Trim();
			}
			else
			{
				_objSalesChallanReturn.Model.VoucherNo = TxtVoucherNo.Text.Trim();
			}

			_objSalesChallanReturn.Model.VTime = _objDate.GetServerDateTime();
			_objSalesChallanReturn.Model.VMiti = TxtMiti.Text;
            _objSalesChallanReturn.Model.ReferenceNo = TxtRefBillNo.Text;
            _objSalesChallanReturn.Model.ReferenceDate = Refdate;
            _objSalesChallanReturn.Model.ReferenceNo = Refmiti;
            _objSalesChallanReturn.Model.LedgerId = Convert.ToInt32(TxtCustomer.Tag.ToString());


			_objSalesChallanReturn.Model.SalesmanId = Convert.ToInt32(TxtSalesman.Tag.ToString());

			if (!string.IsNullOrEmpty(TxtDepartment.Text))
			{
				string[] dept = TxtDepartment.Tag.ToString().Split('|');
				_objSalesChallanReturn.Model.DepartmentId1 = ((dept[0].ToString() == "") ? 0 : Convert.ToInt32(dept[0].ToString()));
				_objSalesChallanReturn.Model.DepartmentId2 = ((dept[1].ToString() == "") ? 0 : Convert.ToInt32(dept[1].ToString()));
				_objSalesChallanReturn.Model.DepartmentId3 = ((dept[2].ToString() == "") ? 0 : Convert.ToInt32(dept[2].ToString()));
				_objSalesChallanReturn.Model.DepartmentId4 = ((dept[3].ToString() == "") ? 0 : Convert.ToInt32(dept[3].ToString()));
			}
			_objSalesChallanReturn.Model.BranchId = ClsGlobal.BranchId;
			_objSalesChallanReturn.Model.CompanyUnitId = ClsGlobal.CompanyUnitId;
			_objSalesChallanReturn.Model.CurrencyId = Convert.ToInt32(TxtCurrency.Tag.ToString());
			_objSalesChallanReturn.Model.CurrencyRate = _CurrencyRate;
			
			decimal.TryParse(LblBasicAmt.Text.Trim(), out decimal BasicAmt);
			_objSalesChallanReturn.Model.BasicAmount = BasicAmt;
			decimal.TryParse(TxtBillTermAmt.Text.Trim(), out decimal TermAmt);
			_objSalesChallanReturn.Model.TermAmount = TermAmt;
			decimal.TryParse(LblNetAmt.Text.Trim(), out decimal NetAmt);
			_objSalesChallanReturn.Model.NetAmount = NetAmt;
			_objSalesChallanReturn.Model.SubLedgerId = Convert.ToInt32(TxtSubledger.Tag.ToString());			
			_objSalesChallanReturn.Model.Remarks = TxtRemarks.Text;

		

			if (!string.IsNullOrEmpty(TxtBillingAddress.Text.ToString().Trim()))
			{
				_objSalesChallanReturn.ModelBillAddress.LedgerId = Convert.ToInt32(TxtCustomer.Tag.ToString());
				_objSalesChallanReturn.ModelBillAddress.BillingAddress = TxtBillingAddress.Text.Trim();
				_objSalesChallanReturn.ModelBillAddress.BillingCity = TxtBillingCity.Text.Trim();
				_objSalesChallanReturn.ModelBillAddress.BillingState = TxtBillingState.Text.Trim();
				_objSalesChallanReturn.ModelBillAddress.BillingCountry = TxtBillingCountry.Text.Trim();
				_objSalesChallanReturn.ModelBillAddress.BillingEmail = TxtBillingEmail.Text.Trim();
				_objSalesChallanReturn.ModelBillAddress.ShippingAddress = TxtShippingAddress.Text.Trim();
				_objSalesChallanReturn.ModelBillAddress.ShippingCity = TxtShippingCity.Text.Trim();
				_objSalesChallanReturn.ModelBillAddress.ShippingState = TxtShippingState.Text.Trim();
				_objSalesChallanReturn.ModelBillAddress.ShippingCountry = TxtShippingCountry.Text.Trim();
				_objSalesChallanReturn.ModelBillAddress.ShippingEmail = TxtShippingEmail.Text.Trim();
			}
			_objSalesChallanReturn.ModelOtherDetails.Transport = TxtTransport.Text.Trim();
			_objSalesChallanReturn.ModelOtherDetails.VehicleNo = TxtVehicleNo.Text.Trim();
			_objSalesChallanReturn.ModelOtherDetails.Package = TxtPackage.Text.Trim();
			_objSalesChallanReturn.ModelOtherDetails.CnNo = TxtCnNo.Text.Trim();

			if (TxtCnDate.Text != "  /  /")
			{
				if (ClsGlobal.DateType == "M")
					_objSalesChallanReturn.ModelOtherDetails.CnDate = _objDate.GetDate(TxtCnDate.Text);
				else
					_objSalesChallanReturn.ModelOtherDetails.CnDate = Convert.ToDateTime(TxtCnDate.Text);
			}
			else
				_objSalesChallanReturn.ModelOtherDetails.CnDate = null;

			_objSalesChallanReturn.ModelOtherDetails.CnFreight = TxtCnFreight.Text.Trim();
			_objSalesChallanReturn.ModelOtherDetails.Advance = TxtAdvance.Text.Trim();
			_objSalesChallanReturn.ModelOtherDetails.BalFreight = TxtBalFreight.Text.Trim();

			if (CmbCnType.Text == "Due")
				_objSalesChallanReturn.ModelOtherDetails.CnType = "D";
			else if (CmbCnType.Text == "To Pay")
				_objSalesChallanReturn.ModelOtherDetails.CnType = "T";
			else if (CmbCnType.Text == "Paid")
				_objSalesChallanReturn.ModelOtherDetails.CnType = "P";
			else if (CmbCnType.Text == "Free")
				_objSalesChallanReturn.ModelOtherDetails.CnType = "F";
			else if (CmbCnType.Text == "")
				_objSalesChallanReturn.ModelOtherDetails.CnType = null;

			_objSalesChallanReturn.ModelOtherDetails.DriverName = TxtDriverName.Text.Trim();
			_objSalesChallanReturn.ModelOtherDetails.DriverLicNo = TxtDriverLicenseNo.Text.Trim();
			_objSalesChallanReturn.ModelOtherDetails.DriverMobileNo = TxtDriverMobileNo.Text.Trim();
			_objSalesChallanReturn.ModelOtherDetails.ContractNo = TxtContactNo.Text.Trim();
			if (TxtContactDate.Text != "  /  /")
			{
				if (ClsGlobal.DateType == "M")
					_objSalesChallanReturn.ModelOtherDetails.ContractDate = _objDate.GetDate(TxtContactDate.Text);
				else
					_objSalesChallanReturn.ModelOtherDetails.ContractDate = Convert.ToDateTime(TxtContactDate.Text);
			}
			else
				_objSalesChallanReturn.ModelOtherDetails.ContractDate = null;

			_objSalesChallanReturn.ModelOtherDetails.ExpInvNo = TxtExportInvNo.Text.Trim();

			if (TxtExportInvDate.Text != "  /  /")
			{
				if (ClsGlobal.DateType == "M")
					_objSalesChallanReturn.ModelOtherDetails.ExpInvDate = _objDate.GetDate(TxtExportInvDate.Text);
				else
					_objSalesChallanReturn.ModelOtherDetails.ExpInvDate = Convert.ToDateTime(TxtExportInvDate.Text);
			}
			else
				_objSalesChallanReturn.ModelOtherDetails.ExpInvDate = null;

			_objSalesChallanReturn.ModelOtherDetails.PoNo = TxtPONo.Text.Trim();

			if (TxtPODate.Text != "  /  /")
			{
				if (ClsGlobal.DateType == "M")
					_objSalesChallanReturn.ModelOtherDetails.PoDate = _objDate.GetDate(TxtPODate.Text);
				else
					_objSalesChallanReturn.ModelOtherDetails.PoDate = Convert.ToDateTime(TxtPODate.Text);
			}
			else
				_objSalesChallanReturn.ModelOtherDetails.PoDate = null;

			_objSalesChallanReturn.ModelOtherDetails.DocBank = TxtDocBank.Text.Trim();
			_objSalesChallanReturn.ModelOtherDetails.LcNo = TxtLCNo.Text.Trim();
			_objSalesChallanReturn.ModelOtherDetails.CustomName = TxtCustomerName.Text.Trim();
			_objSalesChallanReturn.ModelOtherDetails.Cofd = TxtCoFd.Text.Trim();

			SalesChallanReturnDetailsViewModel _SalesChallanReturnDetails = null;
			...
			foreach (DataGridViewRow ro in Grid.Rows)
			{
				...
				{
					_SalesChallanReturnDetails = new SalesChallanReturnDetailsViewModel();
					_SalesChallanReturnDetails.VoucherNo = _objSalesChallanReturn.Model.VoucherNo;
					_SalesChallanReturnDetails.Sno = Grid.Rows.IndexOf(ro) + 1;
					_SalesChallanReturnDetails.ProductId = Convert.ToInt32(ro.Cells["ProductId"].Value.ToString());
					if (ro.Cells["GodownId"].Value != null)
						_SalesChallanReturnDetails.GodownId = string.IsNullOrEmpty(ro.Cells["GodownId"].Value.ToString()) ? 0 : Convert.ToInt32(ro.Cells["GodownId"].Value.ToString());
					else
						_SalesChallanReturnDetails.GodownId = 0;

					if (ro.Cells["AltQty"].Value != null)
					{
						decimal.TryParse(ro.Cells["AltQty"].Value.ToString().Trim(), out decimal _AltQty);
						_SalesChallanReturnDetails.AltQty = _AltQty;
					}
					else
					{
						_SalesChallanReturnDetails.AltQty = 0;
					}

					if (ro.Cells["ProductAltUnitId"].Value != null)
						_SalesChallanReturnDetails.ProductAltUnit = string.IsNullOrEmpty(ro.Cells["ProductUnitId"].Value.ToString()) ? 0 : Convert.ToInt32(ro.Cells["ProductUnitId"].Value.ToString());
					else
						_SalesChallanReturnDetails.ProductAltUnit = 0;
					_SalesChallanReturnDetails.Qty = ro.Cells["Qty"].Value != null ? Convert.ToDecimal(ro.Cells["Qty"].Value.ToString().Trim()) : 0;
					if (ro.Cells["ProductUnitId"].Value != null)
						_SalesChallanReturnDetails.ProductUnit = string.IsNullOrEmpty(ro.Cells["ProductUnitId"].Value.ToString()) ? 0 : Convert.ToInt32(ro.Cells["ProductUnitId"].Value.ToString());
					else
						_SalesChallanReturnDetails.ProductUnit = 0;
					_SalesChallanReturnDetails.ConversionRatio = ro.Cells["QtyConversion"].Value != null ? Convert.ToDecimal(ro.Cells["QtyConversion"].Value.ToString().Trim()) : 0;
					_SalesChallanReturnDetails.SalesRate = ro.Cells["Rate"].Value != null ? Convert.ToDecimal(ro.Cells["Rate"].Value.ToString().Trim()) : 0;
					_SalesChallanReturnDetails.BasicAmount = ro.Cells["BasicAmt"].Value != null ? Convert.ToDecimal(ro.Cells["BasicAmt"].Value.ToString().Trim()) : 0;
					_SalesChallanReturnDetails.TermAmount = ro.Cells["TermsAmt"].Value != null ? Convert.ToDecimal(ro.Cells["TermsAmt"].Value.ToString().Trim()) : 0;
					_SalesChallanReturnDetails.NetAmount = ro.Cells["NetAmt"].Value != null ? Convert.ToDecimal(ro.Cells["NetAmt"].Value.ToString().Trim()) : 0;
					if (ro.Cells["OrderSNo"].Value != null)
						_SalesChallanReturnDetails.OrderSNo = !string.IsNullOrEmpty(ro.Cells["OrderSNo"].Value.ToString()) ? Convert.ToInt32(ro.Cells["OrderSNo"].Value.ToString().Trim()) : 0;
					else
						_SalesChallanReturnDetails.OrderSNo = 0;
					_SalesChallanReturnDetails.OrderNo = ro.Cells["OrderNo"].Value?.ToString().Trim();
					if (ro.Cells["ChallanSNo"].Value != null)
						_SalesChallanReturnDetails.ChallanSNo = !string.IsNullOrEmpty(ro.Cells["ChallanSNo"].Value.ToString()) ? Convert.ToInt32(ro.Cells["ChallanSNo"].Value.ToString().Trim()) : 0;
					else
						_SalesChallanReturnDetails.ChallanSNo = 0;
					_SalesChallanReturnDetails.ChallanNo = ro.Cells["ChallanNo"].Value?.ToString().Trim();
                    if (ro.Cells["DispatchOrderSNo"].Value != null)
                        _SalesChallanReturnDetails.DispatchOrderSNo = ro.Cells["DispatchOrderSNo"].Value != null ? Convert.ToInt32(ro.Cells["DispatchOrderSNo"].Value.ToString().Trim()) : 0;
                    else
                        _SalesChallanReturnDetails.DispatchOrderSNo = 0;
                    _SalesChallanReturnDetails.DispatchOrderNo = ro.Cells["DispatchOrderNo"].Value?.ToString().Trim();
                    if (ro.Cells["QuotationSNo"].Value != null)
                        _SalesChallanReturnDetails.QuotationSNo = ro.Cells["QuotationSNo"].Value != null ? Convert.ToInt32(ro.Cells["QuotationSNo"].Value.ToString().Trim()) : 0;
                    else
                        _SalesChallanReturnDetails.QuotationSNo = 0;
                    _SalesChallanReturnDetails.QuotationNo = ro.Cells["QuotationNo"].Value?.ToString().Trim();
                    _SalesChallanReturnDetails.AdditionalDesc = ro.Cells["AdditionalDesc"].Value != null ? ro.Cells["AdditionalDesc"].Value.ToString() : ro.Cells["Particular"].Value.ToString().Trim();
					_SalesChallanReturnDetails.FreeQty = 0;
					_objSalesChallanReturn.ModelDetails.Add(_SalesChallanReturnDetails);
					if (ro.Cells["TermDetails"].Value != null)
					{
						if (!string.IsNullOrEmpty(ro.Cells["TermDetails"].Value.ToString()))
						{
							...
							for (int i = 0; i < termCodeArr.Count(); i++)
							{
								...
								_objSalesChallanReturn.ModelTerms.Add(_SalesInvoiceTerm);
							}
						}
					}
				}
			...
			if (!string.IsNullOrEmpty(TxtBillTermAmt.Tag.ToString().Trim()))
			{
				...
				{
					_SalesInvoiceTerm = new TermViewModel();
					_SalesInvoiceTerm.VoucherNo = _objSalesChallanReturn.Model.VoucherNo;
			...
			_objSalesChallanReturn.Model.DocId = Convert.ToInt32(TxtVoucherNo.Tag.ToString());
			string output = _objSalesChallanReturn.SaveSalesChallanReturn();
			...
			else
			{
				...
				{
					DocPrintingOption frm = new DocPrintingOption("SCR", "Sales Challan Return", TxtVoucherNo.Text);
					frm.ShowDialog();
				}

				//if (dtVoucherAdj.Rows.Count > 0)
				//{
				//    POSDAL.Entry.Vouchers.VoucherAdjustment objVoucherAdj = new POSDAL.Entry.Vouchers.VoucherAdjustment(SwastikPOS.Util.Database.DBConnection);
				//    POSDAL.Entry.Vouchers.VoucherAdjustmentDetail voucherAdjust = null;
				//    foreach (DataRow ro in dtVoucherAdj.Rows)
				//    {
				//        voucherAdjust = new POSDAL.Entry.Vouchers.VoucherAdjustmentDetail();
				//        if (!string.IsNullOrEmpty(ro["Ref_type"].ToString())) voucherAdjust.Ref_type = ro["Ref_type"].ToString();
				//        if (!string.IsNullOrEmpty(ro["DrV_No"].ToString())) voucherAdjust.DrV_No = ro["DrV_No"].ToString(); else voucherAdjust.DrV_No = null;
				//        if (!string.IsNullOrEmpty(ro["DrV_Source"].ToString())) voucherAdjust.DrV_Source = ro["DrV_Source"].ToString(); else voucherAdjust.DrV_Source = null;
				//        voucherAdjust.Gl_Code = ro["Gl_Code"].ToString();
				//        voucherAdjust.Adj_Amount = Convert.ToDecimal(ro["Adj_Amount"].ToString());
				//        if (!string.IsNullOrEmpty(ro["CrV_No"].ToString())) voucherAdjust.CrV_No = ro["CrV_No"].ToString(); else voucherAdjust.CrV_No = null;
				//        voucherAdjust.DrSno = Convert.ToInt32(ro["DrSno"].ToString());
				//        voucherAdjust.CrSno = Convert.ToInt32(ro["CrSno"].ToString());
				//        if (!string.IsNullOrEmpty(ro["CrV_Source"].ToString())) voucherAdjust.CrV_Source = ro["CrV_Source"].ToString(); else voucherAdjust.CrV_Source = null;
				//        voucherAdjust.Br_Code = ro["Br_Code"].ToString();

				//        objVoucherAdj.voucherAdjustment.Add(voucherAdjust);
				//    }
				//    objVoucherAdj.Save();
				//}
