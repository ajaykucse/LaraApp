...
namespace acmedesktop.DataTransaction.Purchase
{
    public partial class FrmPurchaseOrder : Form
    {
        ...
        int Indexcount, _IsSaveAndPrint = 0, _DocId=0;
        DateTime date;
		...
		IPurchaseIndent _objPurchaseIndent = new ClsPurchaseIndent();
        ...
        public FrmPurchaseOrder()
        {
            ...
            _IsSaveAndPrint = _objDocPrintSetting.CheckIsSaveAndPrint("PO");
        }
        ...
        private void BtnNew_Click(object sender, EventArgs e)
        {
            ...
            {
                Utility.GetVoucherNo1("Purchase Order", TxtVoucherNo, TxtDate, _Tag, "", _DocId);
            }
            else if (TxtVoucherNo.Enabled == true)
            {
                Utility.GetVoucherNo2("Purchase Order", TxtVoucherNo, TxtVoucherNo, _Tag, "", _DocId);
            }
            else if (TxtIndentNo.Enabled == true)
            {
                Utility.GetVoucherNo2("Purchase Order", TxtVoucherNo, TxtQuotationNo, _Tag, "", _DocId);
            }
            else
            {
                Utility.GetVoucherNo1("Purchase Order", TxtVendor, TxtDate, _Tag, "", _DocId);
            }
        }
        ...
        public void ControlEnableDisable(bool btn, bool fld)
        {
            ...
            TxtIndentNo.Enabled = fld;
            Utility.EnableDesibleColor(TxtIndentNo, fld);
            BtnIndentNoSearch.Enabled = fld;
            TxtVoucherNo.Enabled = fld;
            ...
            if (_Tag.ToString() == "NEW" || _Tag.ToString() == "EDIT")
            {
                ...
                if (ClsGlobal.PurchaseIndentControlVal == 'Y')
                {
                    ...
                    TxtIndentNo.BackColor = Color.White;
                    BtnIndentNoSearch.Enabled = true;
                }
                else
                {
                    TxtIndentNo.Enabled = false;
                    TxtIndentNo.BackColor = SystemColors.Control;
                    BtnIndentNoSearch.Enabled = false;
                }
        ...
        {
            DocPrintingOption frm = new DocPrintingOption("PO", "Purchase Order", "");
            frm.ShowDialog();
            frm.Dispose();
        }
        ...
        private void TxtVendor_Validating(object sender, CancelEventArgs e)
        {
            ...
            {
				if (_objCommon.CheckDescriptionDuplicateRecord(TxtVendor.Text, "GeneralLedger", "GlDesc", "LedgerId") != 1)
				{
					MessageBox.Show("Vendor not exist...!", "Mr Solution", MessageBoxButtons.OK, MessageBoxIcon.Information);
					e.Cancel = true;
					TxtVendor.Focus();
					return;
				}
        ...
        private void BtnVoucherNoSearch_Click(object sender, EventArgs e)
        {
            ...
            PickList frmPickList = new PickList("PurchaseOrderVoucher", _SearchKey);
            if (PickList.dt.Rows.Count > 0)
            {
                ...
                if (frmPickList.SelectedList.Count > 0 && _Tag != "NEW")
                {
                    ...
                    DataSet ds = _objPurchaseOrder.GetDataPurchaseOrder(_VoucherNo);
                    ClearFld();
                    SetData(ds);
                }
                frmPickList.Dispose();
        ...
        private void BtnOk_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Order No Cannot Left Blank...!", "Mr Solution", MessageBoxButtons.OK, MessageBoxIcon.Information);
                TxtVoucherNo.Focus();
                return;
            }
			else if (_objCommon.CheckDescriptionDuplicateRecord(TxtVendor.Text, "GeneralLedger", "GlDesc", "LedgerId") != 1)
			{
				MessageBox.Show("Vendor not exist...!", "Mr Solution", MessageBoxButtons.OK, MessageBoxIcon.Information);
				TxtVendor.Focus();
				return;
            ...
            {
                MessageBox.Show("Purchase Order Details Not Found.", "Mr Solution", MessageBoxButtons.OK, MessageBoxIcon.Information);
                Grid.Focus();
                GridControlMode(true);
                return;
            }
            _objPurchaseOrder.Model.Tag = _Tag;
            _objPurchaseOrder.Model.VoucherNo = _VoucherNo;
            ...
            if (_Tag == "NEW")
            {
                string[] VoucherNoDetails = _objCommon.GetVoucherNo(TxtVoucherNo.Tag.ToString(), "Purchase Order", ClsGlobal.BranchId, ClsGlobal.CompanyUnitId);
                _objPurchaseOrder.Model.DocId = ((TxtVoucherNo.Tag.ToString() == "") ? 0 : Convert.ToInt32(TxtVoucherNo.Tag.ToString()));
                _objPurchaseOrder.Model.VoucherNo = (!string.IsNullOrEmpty(VoucherNoDetails[0])) ? VoucherNoDetails[0] : TxtVoucherNo.Text.Trim();
            }
			else
				_objPurchaseOrder.Model.VoucherNo =  TxtVoucherNo.Text.Trim();


			_objPurchaseOrder.Model.VDate = Convert.ToDateTime(TxtDate.Text.ToString());
            _objPurchaseOrder.Model.VTime = Convert.ToDateTime(TxtDate.Text.ToString());
            _objPurchaseOrder.Model.VMiti = TxtMiti.Text;
            _objPurchaseOrder.Model.ReferenceNo = TxtRefNo.Text;
            if (TxtRefDate.Text != "  /  /")
            {
                if (ClsGlobal.DateType == "D")
                {
                    _objPurchaseOrder.Model.ReferenceDate = Convert.ToDateTime(TxtRefDate.Text);
                    _objPurchaseOrder.Model.ReferenceMiti = _objDate.GetMiti(Convert.ToDateTime(TxtRefDate.Text));
                }
                else
                {
                    _objPurchaseOrder.Model.ReferenceDate = _objDate.GetDate(TxtRefDate.Text);
                    _objPurchaseOrder.Model.ReferenceMiti = TxtRefDate.Text;
                }
            }
            else
            {
                _objPurchaseOrder.Model.ReferenceDate = null;
                _objPurchaseOrder.Model.ReferenceMiti = "";
            }
            _objPurchaseOrder.Model.LedgerId = ((TxtVendor.Tag.ToString() == "") ? 0 : Convert.ToInt32(TxtVendor.Tag.ToString()));
            _objPurchaseOrder.Model.SubLedgerId = ((TxtSubledger.Tag.ToString() == "") ? 0 : Convert.ToInt32(TxtSubledger.Tag.ToString()));
            _objPurchaseOrder.Model.SalesmanId = ((TxtSalesman.Tag.ToString() == "") ? 0 : Convert.ToInt32(TxtSalesman.Tag.ToString()));

            if (!string.IsNullOrEmpty(TxtDepartment.Text))
            {
                string[] dept = TxtDepartment.Tag.ToString().Split('|');
                _objPurchaseOrder.Model.DepartmentId1 = ((dept[0].ToString() == "") ? 0 : Convert.ToInt32(dept[0].ToString()));
                _objPurchaseOrder.Model.DepartmentId2 = ((dept[1].ToString() == "") ? 0 : Convert.ToInt32(dept[1].ToString()));
                _objPurchaseOrder.Model.DepartmentId3 = ((dept[2].ToString() == "") ? 0 : Convert.ToInt32(dept[2].ToString()));
                _objPurchaseOrder.Model.DepartmentId4 = ((dept[3].ToString() == "") ? 0 : Convert.ToInt32(dept[3].ToString()));
            }

            _objPurchaseOrder.Model.CurrencyId = ((TxtCurrency.Tag.ToString() == "") ? 0 : Convert.ToInt32(TxtCurrency.Tag.ToString()));
            _objPurchaseOrder.Model.CurrencyRate = ((TxtCurrencyRate.Text.ToString() == "" || Convert.ToDecimal(TxtCurrencyRate.Text.ToString()) == 0) ? 1 : Convert.ToDecimal(TxtCurrencyRate.Text.ToString()));
            _objPurchaseOrder.Model.BranchId = ClsGlobal.BranchId;
            _objPurchaseOrder.Model.CompanyUnitId = ClsGlobal.CompanyUnitId;

            decimal.TryParse(LblBasicAmt.Text, out decimal TotalBasicAmount);
            _objPurchaseOrder.Model.BasicAmount = TotalBasicAmount;
            decimal.TryParse(TxtBillTermAmt.Text, out decimal Totalbillterms);
            _objPurchaseOrder.Model.TermAmount = Totalbillterms;
            decimal.TryParse(LblNetAmt.Text, out decimal TotalNetAmount);
            _objPurchaseOrder.Model.NetAmount = TotalNetAmount;

            _objPurchaseOrder.Model.PartyName = !string.IsNullOrEmpty(CashPartyViewModel.PartyName) ? CashPartyViewModel.PartyName : TxtVendor.Text.Trim();
			_objPurchaseOrder.Model.PartyVatNo = CashPartyViewModel.PartyVatNo;
			_objPurchaseOrder.Model.PartyAddress = CashPartyViewModel.PartyAddress;
			_objPurchaseOrder.Model.PartyMobileNo = CashPartyViewModel.PartyMobileNo;
			if (!string.IsNullOrEmpty(CashPartyViewModel.ChequeNo))
			{
				_objPurchaseOrder.Model.ChequeNo = CashPartyViewModel.ChequeNo;
				_objPurchaseOrder.Model.ChequeDate = Convert.ToDateTime(CashPartyViewModel.ChequeDate);
				_objPurchaseOrder.Model.ChequeMiti = null;
			}
			else
			{
				_objPurchaseOrder.Model.ChequeNo = null;
				_objPurchaseOrder.Model.ChequeDate = null;
				_objPurchaseOrder.Model.ChequeMiti = null;
			}
            _objPurchaseOrder.Model.Remarks = TxtRemarks.Text;
			_objPurchaseOrder.Model.QuotationNo = TxtQuotationNo.Text;
			_objPurchaseOrder.Model.IndentNo = TxtIndentNo.Text;
			_objPurchaseOrder.Model.EnterBy = ClsGlobal.LoginUserCode;
            _objPurchaseOrder.Model.ReconcileBy = "";
            _objPurchaseOrder.Model.ReconcileDate = null;
            _objPurchaseOrder.Model.IsPosted = 0;
            _objPurchaseOrder.Model.PostedBy = "";
            _objPurchaseOrder.Model.PostedDate = null;
            _objPurchaseOrder.Model.IsAuthorized = 0;
            _objPurchaseOrder.Model.AuthorizedBy = "";
            _objPurchaseOrder.Model.AuthorizedDate = null;
            _objPurchaseOrder.Model.AuthorizeRemarks = "";
            _objPurchaseOrder.Model.Gadget = "Desktop";
            _objPurchaseOrder.Model.EntryFromProject = "Normal";
            _objPurchaseOrder.Model.UdfDetails = ClsGlobal.UDFExistingDataTableDetails;
            _objPurchaseOrder.Model.UdfMaster = ClsGlobal.UDFExistingDataMaster;

            if (!string.IsNullOrEmpty(TxtBillingAddress.Text.ToString().Trim()))
            {
                _objPurchaseOrder.ModelBillAddress.LedgerId = Convert.ToInt32(TxtVendor.Tag.ToString());
                _objPurchaseOrder.ModelBillAddress.BillingAddress = TxtBillingAddress.Text.Trim();
                _objPurchaseOrder.ModelBillAddress.BillingCity = TxtBillingCity.Text.Trim();
                _objPurchaseOrder.ModelBillAddress.BillingState = TxtBillingState.Text.Trim();
                _objPurchaseOrder.ModelBillAddress.BillingCountry = TxtBillingCountry.Text.Trim();
                _objPurchaseOrder.ModelBillAddress.BillingEmail = TxtBillingEmail.Text.Trim();
                _objPurchaseOrder.ModelBillAddress.ShippingAddress = TxtShippingAddress.Text.Trim();
                _objPurchaseOrder.ModelBillAddress.ShippingCity = TxtShippingCity.Text.Trim();
                _objPurchaseOrder.ModelBillAddress.ShippingState = TxtShippingState.Text.Trim();
                _objPurchaseOrder.ModelBillAddress.ShippingCountry = TxtShippingCountry.Text.Trim();
                _objPurchaseOrder.ModelBillAddress.ShippingEmail = TxtShippingEmail.Text.Trim();
            }
            _objPurchaseOrder.ModelOtherDetails.Transport = TxtTransport.Text.Trim();
            _objPurchaseOrder.ModelOtherDetails.VehicleNo = TxtVehicleNo.Text.Trim();
            _objPurchaseOrder.ModelOtherDetails.Package = TxtPackage.Text.Trim();
            _objPurchaseOrder.ModelOtherDetails.CnNo = TxtCnNo.Text.Trim();

            if (TxtCnDate.Text != "  /  /")
            {
                if (ClsGlobal.DateType == "M")
                    _objPurchaseOrder.ModelOtherDetails.CnDate = _objDate.GetDate(TxtCnDate.Text);
                else
                    _objPurchaseOrder.ModelOtherDetails.CnDate = Convert.ToDateTime(TxtCnDate.Text);
            }
            else
                _objPurchaseOrder.ModelOtherDetails.CnDate = null;

            _objPurchaseOrder.ModelOtherDetails.CnFreight = TxtCnFreight.Text.Trim();
            _objPurchaseOrder.ModelOtherDetails.Advance = TxtAdvance.Text.Trim();
            _objPurchaseOrder.ModelOtherDetails.BalFreight = TxtBalFreight.Text.Trim();

            if (CmbCnType.Text == "Due")
            {
                _objPurchaseOrder.ModelOtherDetails.CnType = "D";
            }
            else if (CmbCnType.Text == "To Pay")
            {
                _objPurchaseOrder.ModelOtherDetails.CnType = "T";
            }
            else if (CmbCnType.Text == "Paid")
            {
                _objPurchaseOrder.ModelOtherDetails.CnType = "P";
            }
            else if (CmbCnType.Text == "Free")
            {
                _objPurchaseOrder.ModelOtherDetails.CnType = "F";
            }
            else if (CmbCnType.Text == "")
            {
                _objPurchaseOrder.ModelOtherDetails.CnType = null;
            }
            _objPurchaseOrder.ModelOtherDetails.DriverName = TxtDriverName.Text.Trim();
            _objPurchaseOrder.ModelOtherDetails.DriverLicNo = TxtDriverLicenseNo.Text.Trim();
            _objPurchaseOrder.ModelOtherDetails.DriverMobileNo = TxtDriverMobileNo.Text.Trim();
            _objPurchaseOrder.ModelOtherDetails.ContractNo = TxtContactNo.Text.Trim();
            if (TxtContactDate.Text != "  /  /")
            {
                if (ClsGlobal.DateType == "M")
                    _objPurchaseOrder.ModelOtherDetails.ContractDate = _objDate.GetDate(TxtContactDate.Text);
                else
                    _objPurchaseOrder.ModelOtherDetails.ContractDate = Convert.ToDateTime(TxtContactDate.Text);
            }
            else
                _objPurchaseOrder.ModelOtherDetails.ContractDate = null;

            _objPurchaseOrder.ModelOtherDetails.ExpInvNo = TxtExportInvNo.Text.Trim();

            if (TxtExportInvDate.Text != "  /  /")
            {
                if (ClsGlobal.DateType == "M")
                    _objPurchaseOrder.ModelOtherDetails.ExpInvDate = _objDate.GetDate(TxtExportInvDate.Text);
                else
                    _objPurchaseOrder.ModelOtherDetails.ExpInvDate = Convert.ToDateTime(TxtExportInvDate.Text);
            }
            else
                _objPurchaseOrder.ModelOtherDetails.ExpInvDate = null;

            _objPurchaseOrder.ModelOtherDetails.PoNo = TxtPONo.Text.Trim();

            if (TxtPODate.Text != "  /  /")
            {
                if (ClsGlobal.DateType == "M")
                    _objPurchaseOrder.ModelOtherDetails.PoDate = _objDate.GetDate(TxtPODate.Text);
                else
                    _objPurchaseOrder.ModelOtherDetails.PoDate = Convert.ToDateTime(TxtPODate.Text);
            }
            else
                _objPurchaseOrder.ModelOtherDetails.PoDate = null;

            _objPurchaseOrder.ModelOtherDetails.DocBank = TxtDocBank.Text.Trim();
            _objPurchaseOrder.ModelOtherDetails.LcNo = TxtLCNo.Text.Trim();
            _objPurchaseOrder.ModelOtherDetails.CustomName = TxtCustomerName.Text.Trim();
            _objPurchaseOrder.ModelOtherDetails.Cofd = TxtCoFd.Text.Trim();

            PurchaseOrderDetailsViewModel _PurchaseOrderDetails = null;
            TermViewModel _PurchaseOrderTerm = null;
            int dc = Grid.Rows.Count;
            foreach (DataGridViewRow ro in Grid.Rows)
            {
                if (ro.Cells["Particular"].Value != null)
                {
                    _PurchaseOrderDetails = new PurchaseOrderDetailsViewModel();
                    _PurchaseOrderDetails.VoucherNo = _objPurchaseOrder.Model.VoucherNo;
                    _PurchaseOrderDetails.Sno = Grid.Rows.IndexOf(ro) + 1;
                    _PurchaseOrderDetails.ProductId = Convert.ToInt32(ro.Cells["ProductId"].Value.ToString());
                    if (ro.Cells["GodownId"].Value != null)
                        _PurchaseOrderDetails.GodownId = string.IsNullOrEmpty(ro.Cells["GodownId"].Value.ToString()) ? 0 : Convert.ToInt32(ro.Cells["GodownId"].Value.ToString());
                    else
                        _PurchaseOrderDetails.GodownId = 0;


                    if (ro.Cells["AltQty"].Value != null)
                    {
                        decimal.TryParse(ro.Cells["AltQty"].Value.ToString().Trim(), out decimal _AltQty);
                        _PurchaseOrderDetails.AltQty = _AltQty;
                    }
                    else
                    {
                        _PurchaseOrderDetails.AltQty = 0;
                    }

                    if (ro.Cells["ProductAltUnitId"].Value != null)
                        _PurchaseOrderDetails.ProductAltUnitId = string.IsNullOrEmpty(ro.Cells["ProductAltUnitId"].Value.ToString()) ? 0 : Convert.ToInt32(ro.Cells["ProductAltUnitId"].Value.ToString());
                    else
                        _PurchaseOrderDetails.ProductAltUnitId = 0;

                    _PurchaseOrderDetails.Qty = ro.Cells["Qty"].Value != null ? Convert.ToDecimal(ro.Cells["Qty"].Value.ToString().Trim()) : 0;

                    if (ro.Cells["ProductUnitId"].Value != null)
                        _PurchaseOrderDetails.ProductUnitId = string.IsNullOrEmpty(ro.Cells["ProductUnitId"].Value.ToString()) ? 0 : Convert.ToInt32(ro.Cells["ProductUnitId"].Value.ToString());
                    else
                        _PurchaseOrderDetails.ProductUnitId = 0;

                    _PurchaseOrderDetails.ConversionRatio = ro.Cells["QtyConversion"].Value != null ? Convert.ToDecimal(ro.Cells["QtyConversion"].Value.ToString().Trim()) : 0;
                    _PurchaseOrderDetails.PurchaseRate = ro.Cells["Rate"].Value != null ? Convert.ToDecimal(ro.Cells["Rate"].Value.ToString().Trim()) : 0;
                    _PurchaseOrderDetails.BasicAmount = ro.Cells["BasicAmt"].Value != null ? Convert.ToDecimal(ro.Cells["BasicAmt"].Value.ToString().Trim()) : 0;
                    _PurchaseOrderDetails.TermAmount = ro.Cells["TermsAmt"].Value != null ? Convert.ToDecimal(ro.Cells["TermsAmt"].Value.ToString().Trim()) : 0;
                    _PurchaseOrderDetails.NetAmount = ro.Cells["NetAmt"].Value != null ? Convert.ToDecimal(ro.Cells["NetAmt"].Value.ToString().Trim()) : 0;
					if (ro.Cells["QuotationSNo"].Value != null)
						_PurchaseOrderDetails.QuotationSNo = ro.Cells["QuotationSNo"].Value != null ? Convert.ToInt32(ro.Cells["QuotationSNo"].Value.ToString().Trim()) : 0;
					else
						_PurchaseOrderDetails.QuotationSNo = 0;

				_PurchaseOrderDetails.QuotationNo = ro.Cells["QuotationNo"].Value?.ToString().Trim();
					if (ro.Cells["IndentSNo"].Value != null)
						_PurchaseOrderDetails.IndentSNo = ro.Cells["IndentSNo"].Value != null ? Convert.ToInt32(ro.Cells["IndentSNo"].Value.ToString().Trim()) : 0;
					else
						_PurchaseOrderDetails.IndentSNo = 0;

					_PurchaseOrderDetails.IndentNo = ro.Cells["IndentNo"].Value?.ToString().Trim();
					_PurchaseOrderDetails.AdditionalDesc = ro.Cells["AdditionalDesc"].Value != null ? ro.Cells["AdditionalDesc"].Value.ToString() : ro.Cells["Particular"].Value.ToString().Trim();
                    _PurchaseOrderDetails.FreeQty = 0;
                    _objPurchaseOrder.ModelDetails.Add(_PurchaseOrderDetails);
                    ...
                    if (ro.Cells["TermDetails"].Value != null)
                    {
                        if (!string.IsNullOrEmpty(ro.Cells["TermDetails"].Value.ToString()))
                        {
                            ...
                            {
                                _PurchaseOrderTerm = new TermViewModel();
                                _PurchaseOrderTerm.VoucherNo = TxtVoucherNo.Text;
                                _PurchaseOrderTerm.TermId = Convert.ToInt32(termCodeArr[i]);
                                _PurchaseOrderTerm.Sno = Grid.Rows.IndexOf(ro) + 1;
                                _PurchaseOrderTerm.ProductId = Convert.ToInt32(ro.Cells["ProductId"].Value.ToString());
                                _PurchaseOrderTerm.TermType = "P";
                                _PurchaseOrderTerm.PTSign = termSignArr[i];
                                _PurchaseOrderTerm.TermRate = Convert.ToDecimal(termRatePercentArr[i]);
                                _PurchaseOrderTerm.TermAmt = Convert.ToDecimal(termAmountntArr[i]);
                                _objPurchaseOrder.ModelTerms.Add(_PurchaseOrderTerm);
                            }
                        }
                    }
                }
			...
			if (!string.IsNullOrEmpty(TxtBillTermAmt.Tag.ToString().Trim()))
			{
				...
				{
					_PurchaseOrderTerm = new TermViewModel();
					_PurchaseOrderTerm.VoucherNo = _objPurchaseOrder.Model.VoucherNo;
					_PurchaseOrderTerm.TermId = Convert.ToInt32(termCodeArr1[i]);
					_PurchaseOrderTerm.Sno = 0;
					_PurchaseOrderTerm.ProductId = 0;
					_PurchaseOrderTerm.TermType = "B";
					_PurchaseOrderTerm.PTSign = termSignArr[i];
					...
					_PurchaseOrderTerm.TermRate = _termRatePercentArr1;
					_PurchaseOrderTerm.TermAmt = _termAmountArr1;
					_objPurchaseOrder.ModelTerms.Add(_PurchaseOrderTerm);
				}

			}

            string result = _objPurchaseOrder.SavePurchaseOrder();
            ...
            {
                if (_Tag == "NEW")
                    MessageBox.Show("Order number : " + TxtVoucherNo.Text + " has been generated.", "Mr Solution", MessageBoxButtons.OK, MessageBoxIcon.Information);
                else if (_Tag == "EDIT")
                    MessageBox.Show("Order number : " + TxtVoucherNo.Text + " has been updated.", "Mr Solution", MessageBoxButtons.OK, MessageBoxIcon.Information);
                else if (_Tag == "DELETE")
                    MessageBox.Show("Order number : " + TxtVoucherNo.Text + " has been deleted.", "Mr Solution", MessageBoxButtons.OK, MessageBoxIcon.Information);

                if (_IsSaveAndPrint == 1)
                {
                    DocPrintingOption frm = new DocPrintingOption("PO", "Purchase Order", TxtVoucherNo.Text);
                    frm.ShowDialog();
                }
                ClearFld();
				if (this._Tag == "NEW")
        ...
        private void TxtOrderNo_Validating(object sender, CancelEventArgs e)
        {
            ...
            if (_Tag == "NEW")
            {
                if (!string.IsNullOrEmpty(_objPurchaseOrder.IsOrderNumberExists(TxtVoucherNo.Text)))
                {
                    MessageBox.Show("Order Number Already Exist...!", "Mr. Solution", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    e.Cancel = true;
                    return;
                }
            }
			else if (_Tag == "EDIT" || _Tag == "DELETE")
			{
				if (string.IsNullOrEmpty(_objPurchaseOrder.IsOrderNumberExists(TxtVoucherNo.Text)))
				{
					MessageBox.Show("Purchase order number does not exist.", "Mr. Solution", MessageBoxButtons.OK, MessageBoxIcon.Information);
					e.Cancel = true;
					return;
				}
				else
				{
					...
					if (this._VoucherNo != TxtVoucherNo.Text.Trim())
					{
						this._VoucherNo = TxtVoucherNo.Text.Trim();
						 ds = _objPurchaseOrder.GetDataPurchaseOrder(this._VoucherNo);
						ClearFld();
						SetData(ds);
					}
					string _vno1 = _objPurchaseOrder.IsOrderUsedInChallan(TxtVoucherNo.Text);
					if (!string.IsNullOrEmpty(_vno1))
					{
						MessageBox.Show("This order number is used in Purchase challan.", "Mr. Solution", MessageBoxButtons.OK, MessageBoxIcon.Information);
						_VoucherNo = _vno1; _isSalesQuotationUsed = true; BtnOk.Enabled = false;
						//ds = _objPurchaseOrder.GetDataPurchaseOrder(_vno1);
						//SetData(ds);
						return;
					}

					string _vno2 = _objPurchaseOrder.IsOrderUsedInBill(TxtVoucherNo.Text);
					if (!string.IsNullOrEmpty(_vno2))
					{
						MessageBox.Show("This order number is used in Purchase invoice.", "Mr. Solution", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void TxtRefDate_Validating(object sender, CancelEventArgs e)
        {
		...
		private void BtnIndentNoSearch_Click(object sender, EventArgs e)
		{
			Common.PickList frmPickList = new Common.PickList("PurchaseIndent", _SearchKey);
			if (Common.PickList.dt.Rows.Count > 0)
			{
				frmPickList.ShowDialog();
				if (frmPickList.SelectedList.Count > 0)
				{
					TxtIndentNo.Text = frmPickList.SelectedList[0]["VoucherNo"].ToString().Trim();
					TxtIndentNo.SelectAll();
				}
				frmPickList.Dispose();
			}
			else
		...
		{
			if (_Tag == "" || ActiveControl == TxtIndentNo) return;
			if (TxtIndentNo.Enabled == false) return;
			if (ClsGlobal.PurchaseMQuotationControlVal == 'Y' && string.IsNullOrEmpty(TxtIndentNo.Text))
			{
			...
			if (!string.IsNullOrEmpty(TxtIndentNo.Text.Trim()))
			{
				if (this._Tag == "NEW")
				{
					DataSet ds = _objPurchaseIndent.GetDataPurchaseIndentVoucher(TxtIndentNo.Text.Trim());
					Grid.Rows.Clear();
					Grid.Rows.Add();
					SetDataPurchaseIndent(ds);
				}
		...
		private void TxtQuotationNo_Validating(object sender, CancelEventArgs e)
		{
			...
			{
					DataSet ds = _objPurchaseQuotation.GetDataPurchaseQuotationForOrder(TxtQuotationNo.Text.Trim(),TxtVoucherNo.Text,"PO");
					Grid.Rows.Clear();
					Grid.Rows.Add();
					SetDataPurchaseQuotation(ds);

				TxtIndentNo.Enabled = false;
				Utility.EnableDesibleColor(TxtIndentNo, false);
				BtnIndentNoSearch.Enabled = false;
			}
		...
		{
			_SearchKey = e.KeyCode.ToString() == "F1" || e.KeyCode.ToString() == "Escape" ? string.Empty : ((char)e.KeyCode).ToString();
			ClsGlobal.MyKeyDown((char)e.KeyCode, e.KeyValue, e.KeyData.ToString(), "DELETE", _SearchKey, TxtIndentNo, BtnIndentNoSearch, false);
		}
        ...
        private void SetData(DataSet ds)
        {
			...
			TxtIndentNo.Text= dtMaster.Rows[0]["IndentNo"].ToString();
            ...
            foreach (DataRow drDetails in dtDetails.Rows)
            {
				...
				Grid.Rows[Grid.Rows.Count - 1].Cells["IndentNo"].Value = drDetails["IndentNo"].ToString();
				Grid.Rows[Grid.Rows.Count - 1].Cells["IndentSNo"].Value = drDetails["IndentSNo"].ToString();
				Grid.Rows.Add();
            }
		...
		private void SetDataPurchaseIndent(DataSet ds)
		{
			DataTable dtMaster = ds.Tables[0];
			DataTable dtDetails = ds.Tables[1];
			...
			TxtVendor.Text = dtMaster.Rows[0]["RequestedBy"].ToString();
			TxtRemarks.Text = dtMaster.Rows[0]["Remarks"].ToString();
			foreach (DataRow drDetails in dtDetails.Rows)
			{
				Grid.Rows[Grid.Rows.Count - 1].Cells["SNo"].Value = Grid.Rows.Count.ToString();
				...
				Grid.Rows[Grid.Rows.Count - 1].Cells["ProductUnitId"].Value = drDetails["ProductUnit"].ToString();
				Grid.Rows[Grid.Rows.Count - 1].Cells["QtyConversion"].Value = ClsGlobal.DecimalFormate(Convert.ToDecimal(drDetails["ConversionRatio"].ToString()), 1, ClsGlobal._AmountDecimalFormat).ToString();
				decimal purchaserate = Convert.ToDecimal(drDetails["PurchaseRate"].ToString());
				Grid.Rows[Grid.Rows.Count - 1].Cells["Rate"].Value = purchaserate;
				Grid.Rows[Grid.Rows.Count - 1].Cells["BasicAmt"].Value = ClsGlobal.DecimalFormate((Convert.ToDecimal(drDetails["Qty"].ToString()) * purchaserate), 1, ClsGlobal._AmountDecimalFormat).ToString();
				Grid.Rows[Grid.Rows.Count - 1].Cells["TermsAmt"].Value = ClsGlobal.DecimalFormate(0, 1, ClsGlobal._AmountDecimalFormat).ToString();
				Grid.Rows[Grid.Rows.Count - 1].Cells["NetAmt"].Value = ClsGlobal.DecimalFormate((Convert.ToDecimal(drDetails["Qty"].ToString()) * purchaserate), 1, ClsGlobal._AmountDecimalFormat).ToString();
				Grid.Rows[Grid.Rows.Count - 1].Cells["TermDetails"].Value = "";
				Grid.Rows[Grid.Rows.Count - 1].Cells["AltStockQty"].Value = ClsGlobal.DecimalFormate(Convert.ToDecimal(drDetails["AltQty"].ToString()), 1, ClsGlobal._AltQtyDecimalFormat).ToString();
				Grid.Rows[Grid.Rows.Count - 1].Cells["StockQty"].Value = ClsGlobal.DecimalFormate(Convert.ToDecimal(drDetails["Qty"].ToString()), 1, ClsGlobal._QtyDecimalFormat).ToString();
				Grid.Rows[Grid.Rows.Count - 1].Cells["IndentNo"].Value = drDetails["VoucherNo"].ToString();
				Grid.Rows[Grid.Rows.Count - 1].Cells["IndentSNo"].Value = drDetails["Sno"].ToString();
