...
{
    public class ClsPurchaseChallanReturn : IPurchaseChallanReturn
    {
        ActiveDataAccess.ActiveDataAccess DAL;
        public PurchaseChallanReturnMasterViewModel Model { get; set; }
        public List<PurchaseChallanReturnDetailsViewModel> ModelDetails { get; set; }
        ...
        public string SavePurchaseChallanReturn()
        {
            ...
            if (Model.Tag == "NEW")
            {
                strSql.Append("INSERT INTO ERP.PurchaseChallanReturnMaster([VoucherNo], [VDate], [VTime], [VMiti],[ReferenceNo], [ReferenceDate], [ReferenceMiti],  \n");
                ...
                strSql.Append("INSERT INTO [ERP].[PurchaseChallanReturnOtherDetails]([VoucherNo],[Transport],[VehicleNo],[Package],[CnNo],[CnDate],[CnFreight],[CnType]\n");
                ...
                strSql.Append("INSERT INTO[ERP].[PurchaseChallanReturnBillingAddress] ([VoucherNo],[LedgerId],[BillingAddress],[BillingCity],[BillingState],[BillingCountry],[BillingEmail],[ShippingAddress],[ShippingCity],[ShippingState],[ShippingCountry],[ShippingEmail],[DeliveryDate],[Remarks])\n");
            ...
            {
                strSql.Append("UPDATE ERP.PurchaseChallanReturnMaster SET VDate='" + Model.VDate.ToString("yyyy-MM-dd") + "',VTime='" + Model.VDate.ToString("yyyy-MM-dd") + ' ' + DateTime.Now.ToShortTimeString() + "',VMiti='" + Model.VMiti.ToString() + "',ReferenceNo= " + ((Model.ReferenceNo == "") ? "null" : "'" + Model.ReferenceNo + "'") + ",ReferenceDate='" + Model.VDate.ToString("yyyy-MM-dd") + "', ReferenceMiti='" + Model.VMiti.ToString() + "',LedgerId= " + ((Model.LedgerId == 0) ? "null" : "'" + Model.LedgerId + "'") + ", \n");
            ...
            foreach (PurchaseChallanReturnDetailsViewModel det in ModelDetails)
            {
                strSql.Append("INSERT INTO ERP.PurchaseChallanReturnDetails(VoucherNo ,Sno,ProductId ,ProductAltUnit ,ProductUnit ,GodownId ,AltQty ,Qty ,PurchaseRate ,BasicAmount ,TermAmount ,NetAmount ,LocalNetAmount, AdditionalDesc ,ConversionRatio ,FreeQty ,FreeQtyUnit ,OrderNo ,OrderSNo ,QuotationNo ,QuotationSNo,ChallanInvoiceNo, ChallanInvoiceSno) \n");
                strSql.Append("Select @VoucherNo, '" + det.Sno + "', '" + det.ProductId + "'," + ((det.ProductAltUnit == 0) ? "null" : "'" + det.ProductAltUnit + "'") + ", " + ((det.ProductUnit == 0) ? "null" : "'" + det.ProductUnit + "'") + ", " + ((det.GodownId == 0) ? "null" : "'" + det.GodownId + "'") + ",'" + det.AltQty + "', '" + det.Qty + "', '" + det.PurchaseRate + "','" + det.BasicAmount + "', '" + det.TermAmount + "', '" + det.NetAmount + "', '" + det.LocalNetAmount + "','" + det.AdditionalDesc + "', '" + det.ConversionRatio + "' ,'" + det.FreeQty + "' ," + ((det.FreeQtyUnit == 0) ? "null" : "'" + det.FreeQtyUnit + "'") + " ,'" + det.OrderNo + "' ,'" + det.OrderSNo + "' ," + "'" + det.QuotationNo + "' ,'" + det.QuotationSNo + "','" + det.ChallanInvoiceNo + "' ,'" + det.ChallanInvoiceSNo + "' \n");

            }
            foreach (TermViewModel det in ModelTerms)
            {
                strSql.Append("INSERT INTO ERP.PurchaseChallanReturnTerm(VoucherNo ,Sno ,ProductId ,TermId ,TermType ,PTSign ,TermRate ,TermAmt ,LocalTermAmt ) \n");
            ...
            {
                strSql.Append("DELETE FROM ERP.UDFDataEntry WHERE VoucherNo=@VoucherNo AND EntryModule='PCR' and SNO <> 0 \n");
                int _s = 0;
                foreach (DataRow ro in Model.UdfDetails.Rows)
                {
                    ...
                    for (int i = 0; i < (Model.UdfDetails.Columns.Count - 1) / 2; i++)
                    {
                        ...
                        strSql.Append("Select @VoucherNo,'PCR','" + ro[0].ToString() + "', ");
            ...
            if (Model.UdfMaster.Rows.Count > 0)
            {
                ...
                foreach (DataRow ro in Model.UdfMaster.Rows)
                {
                    ...
                    {
                        strSql.Append("INSERT INTO ERP.UDFDataEntry(VoucherNo,EntryModule,SNO,UDFCode,UDFData,ProductId) \n");
                        strSql.Append("Select @VoucherNo,'PCR','0','" + ro[j].ToString() + "',");
                        j++;
            ...
            //BT Posting
            strSql.Append("insert into erp.PurchaseChallanReturnTerm(VoucherNo, TermId, Sno, ProductId, TermType, PTSign, TermRate, TermAmt, LocalTermAmt)  \n");
            strSql.Append("(Select @VoucherNo as VoucherNo, TermId as TermId, Sno, Pbd1.ProductId, 'BT' as TermType, PTSign, TermRate  \n");
            strSql.Append(", isnull(abs(sum((Amt * Pbd1.Bamt1) / Bamt)), 0) as TermAmt1, 0 as LocalTermAmt from erp.PurchaseChallanReturnMaster as sm,  \n");
            strSql.Append("(Select Sno, SD.ProductId, SD.VoucherNo, sum(Case When SD.NetAmount <> 0 then SD.NetAmount else SD.BasicAmount end) as Bamt1 from erp.PurchaseChallanReturnDetails as SD, erp.PurchaseChallanReturnMaster as SM  where SD.VoucherNo = SM.VoucherNo  \n");
            strSql.Append("group by SD.ProductId, SD.VoucherNo, Sno) as Pbd1, (select SD.VoucherNo, CASE WHEN  sum(Case when SD.NetAmount <> 0 then SD.NetAmount * CurrencyRate else SD.BasicAmount * CurrencyRate end) = 0 THEN 1 ELSE sum(Case when SD.NetAmount<>0 then SD.NetAmount* CurrencyRate else SD.BasicAmount* CurrencyRate end ) END as Bamt from erp.PurchaseChallanReturnDetails as SD,erp.PurchaseChallanReturnMaster as SM  where SD.VoucherNo = SM.VoucherNo group by SD.VoucherNo) as Pbd,   \n");
            strSql.Append("(Select SD.VoucherNo,SD.TermId,SD.TermRate,sum((case when Ptm.PTSign = '+' then(SD.TermAmt * CurrencyRate) else -(SD.TermAmt * CurrencyRate) end)) as Amt,Ptm.PTSign from erp.PurchaseChallanReturnTerm as SD, erp.PurchaseChallanReturnMaster as Sm,erp.PurchaseBillingTerm as Ptm  \n");
            strSql.Append("where SD.VoucherNo = SM.VoucherNo and SD.TermId = Ptm.TermId and ProductId is Null and Basis <> 'Q' and Exists(Select* from erp.PurchaseChallanReturnDetails as Pbd where SD.VoucherNo = Pbd.VoucherNo group by ProductId) and SD.VoucherNo = @VoucherNo  \n");
            ...
            strSql.Append("from erp.PurchaseChallanReturnMaster as Sm,(Select VoucherNo, Sum(Qty) as TotQty from erp.PurchaseChallanReturnDetails group by VoucherNo) as SD,   \n");
            strSql.Append("(select Sno, ProductId, VoucherNo, sum(Qty) as Bamt from erp.PurchaseChallanReturnDetails group by VoucherNo,ProductId,Sno) as Pbd,   \n");
            ...
            strSql.Append("from erp.PurchaseChallanReturnTerm as SD,erp.PurchaseChallanReturnMaster as Sm,erp.PurchaseBillingTerm as Ptm where SD.VoucherNo = SM.VoucherNo and SD.TermId = Ptm.TermId and ProductId is Null and Basis = 'Q'  \n");
            strSql.Append("and Exists(Select* from erp.PurchaseChallanReturnDetails as Pbd where SD.VoucherNo = Pbd.VoucherNo group by ProductId) and SD.VoucherNo = @VoucherNo group by SD.VoucherNo,SD.TermId,SD.TermRate,Ptm.PTSign) as Trm  \n");
            ...
            strSql.Append("Update erp.PurchaseChallanReturnTerm set LocalTermAmt = TermAmt * CurrencyRate from erp.PurchaseChallanReturnMaster where erp.PurchaseChallanReturnTerm.VoucherNo = erp.PurchaseChallanReturnMaster.VoucherNo and erp.PurchaseChallanReturnMaster.VoucherNo = @VoucherNo  \n");
            ...
            strSql.Append("'PCR' as Source,SID.GodownId,0 as DocumentValue, SIM.TermAmount as BillTerm,0 as StockValue,0 as TmpStockValue, 0 as ExtraFreeQty, 0 as ExtraStockFreeQty,null as  ExtraFreeUnit,'' as RefVoucherNo, '' as ReferenceSource, 0 as  Issueqty, 0 as IsBillCancel \n");
            strSql.Append("from ERP.PurchaseChallanReturnMaster SIM Left Outer Join ERP.PurchaseChallanReturnDetails [SID] on[SID].VoucherNo= SIM.VoucherNo where ProductId in (Select ProductId from erp.Product where [ProductCategory] <> 'S') \n");
        ...
        {
            DataTable dt = DAL.ExecuteDataset(CommandType.Text, "select VoucherNo from erp.PurchaseChallanReturnMaster WHERE VoucherNo='" + VoucherNo.Trim() + "'").Tables[0];
            if (dt.Rows.Count == 0)
                return "";
            else
        ...
        public DataSet GetDataPurchaseChallanReturnVoucher(string VoucherNo)
        {
            StringBuilder strSql = new StringBuilder();
            ...
            strSql.Append("from erp.PurchaseChallanReturnMaster PIM  \n");
            ...
            strSql.Append("Select[PID].*,ProductShortName, ProductDesc,PD.AltConv,PAU.ProductUnitShortName as ProductAltUnitDesc,PU.ProductUnitShortName as ProductUnitDesc,GodownDesc from erp.PurchaseChallanReturnDetails[PID] \n");
            ...
            strSql.Append("Select * from erp.PurchaseChallanReturnTerm  \n");
            if (!string.IsNullOrEmpty(VoucherNo))
                strSql.Append(" where voucherNo='" + VoucherNo + "' \n");

            strSql.Append("select* from[ERP].[UDFDataEntry] where VoucherNo = '" + VoucherNo + "' and EntryModule = 'PCR'\n");

            strSql.Append("Select * from ERP.PurchaseChallanReturnOtherDetails  where VoucherNo = '" + VoucherNo + "' \n");
            strSql.Append("Select * from ERP.PurchaseChallanReturnBillingAddress  where VoucherNo = '" + VoucherNo + "' \n");

            return DAL.ExecuteDataset(CommandType.Text, strSql.ToString());
        }

        public DataSet GetDataPurchaseChallanVoucherDataForReturn(string VoucherNo)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(" Select PIM.*,GLDesc,GlCategory,SubledgerDesc,SalesmanDesc,D1.DepartmentId as DepartmentId1,D1.DepartmentDesc as DepartmentDesc1,D2.DepartmentId as DepartmentId2,D2.DepartmentDesc as DepartmentDesc2,D3.DepartmentId as DepartmentId3,D3.DepartmentDesc as DepartmentDesc3,D4.DepartmentId as DepartmentId4,D4.DepartmentDesc as DepartmentDesc4   \n");
            strSql.Append(" , CurrencyDesc, BranchName, CU.CmpUnitName \n");
            ...
            strSql.Append("ProductShortName, ProductDesc, ProductPrintingName, P.AltConv, PAU.ProductUnitShortName as ProductAltUnitDesc, PU.ProductUnitShortName as ProductUnitDesc, GodownDesc, P.IsTaxable, OrderNo, OrderSNo, QuotationNo, QuotationSNo \n");
            strSql.Append("FROM ERP.PurchaseChallanDetails as PID \n");
            strSql.Append("Left Outer join( \n");
            strSql.Append("select ChallanInvoiceNo, ChallanInvoiceSNo, sum(Qty) as CQty from ERP.PurchaseChallanReturnDetails where ChallanInvoiceNo is not Null \n");
            strSql.Append("group by ChallanInvoiceNo, ChallanInvoiceSNo \n");
            strSql.Append(") as PIR on PIR.ChallanInvoiceNo = PID.VoucherNo and PIR.ChallanInvoiceSNo = PID.SNo \n");
            ...
            strSql.Append("PID.AdditionalDesc, PID.TermAmount, PID.NetAmount,PID.OrderNo, PID.OrderSNo, PID.QuotationNo, PID.QuotationSNo, P.IsSerialWise, P.IsBatchwise, PID.ConversionRatio, PID.FreeQtyUnit, \n");
        ...
        public class PurchaseChallanReturnMasterViewModel
        {
        ...
        public class PurchaseChallanReturnDetailsViewModel
        {
            public string VoucherNo { get; set; }
            ...
            public int ChallanInvoiceSNo { get; set; }
