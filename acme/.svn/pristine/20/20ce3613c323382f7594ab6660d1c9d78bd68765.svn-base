...
{
    public partial class FrmPurchaseQuotation : Form
    {
        ...
        int _IsSaveAndPrint = 0; int Indexcount, _DocId=0;
		...
		public FrmPurchaseQuotation()
        {
            ...
            _IsSaveAndPrint = _objDocPrintSetting.CheckIsSaveAndPrint("PQ");
        }
        ...
        private void BtnNew_Click(object sender, EventArgs e)
        {
            ...
            {
                Utility.GetVoucherNo1("Purchase Quotation", TxtVoucherNo, TxtDate, _Tag, "", _DocId);
            }
            else if (TxtIndentNo.Enabled == true)
            {
                Utility.GetVoucherNo2("Purchase Quotation", TxtVoucherNo, TxtIndentNo, _Tag, "", _DocId);
            }
            else
            {
                Utility.GetVoucherNo1("Purchase Quotation", TxtVendor, TxtDate, _Tag, "", _DocId);
            }
        }
        ...
        private void BtnDelete_Click(object sender, EventArgs e)
        {
            ...
            Text = "Purchase Quotation [DELETE]";
        ...
        private void BtnUDF_Click(object sender, EventArgs e)
        {
            ...
            //else
            //{
            //    FrmUDFMasterEntry frm = new FrmUDFMasterEntry("Purchase Master Global", _VoucherNo, "PQ");
            //    frm.ShowDialog();
            //}
            BtnOk.Focus();
        }
        ...
        private void BtnOk_Click(object sender, EventArgs e)
        {
            ...
            if (_NetAmt < 0)
            {
                MessageBox.Show("Purchase quotation negative should not be save.", "Mr Solution", MessageBoxButtons.OK, MessageBoxIcon.Information);
                BtnOk.Focus();
                return;
            }
            ...
            _objPurchaseQuotation.Model.EntryFromProject = "Normal";
            _objPurchaseQuotation.Model.Gadget = "Desktop";

           // _objPurchaseQuotation.Model.PartyName = TxtVendor.Text;
            _objPurchaseQuotation.Model.ChequeDate = null;
            _objPurchaseQuotation.Model.UdfDetails = ClsGlobal.UDFExistingDataTableDetails;
            _objPurchaseQuotation.Model.UdfMaster = ClsGlobal.UDFExistingDataMaster;
            _objPurchaseQuotation.Model.VDate = Convert.ToDateTime(TxtDate.Text);
            _objPurchaseQuotation.Model.Tag = _Tag;

            if (_Tag == "NEW")
            {
                string[] VoucherNoDetails = _objCommon.GetVoucherNo(TxtVoucherNo.Tag.ToString(), "Purchase Quotation", ClsGlobal.BranchId, ClsGlobal.CompanyUnitId);
                _objPurchaseQuotation.Model.DocId = ((TxtVoucherNo.Tag.ToString() == "") ? 0 : Convert.ToInt32(TxtVoucherNo.Tag.ToString()));
                _objPurchaseQuotation.Model.VoucherNo = (!string.IsNullOrEmpty(VoucherNoDetails[0])) ? VoucherNoDetails[0] : TxtVoucherNo.Text.Trim();
            }
			else
				_objPurchaseQuotation.Model.VoucherNo = TxtVoucherNo.Text.Trim();


			_objPurchaseQuotation.Model.VTime = _objDate.GetServerDateTime();
            _objPurchaseQuotation.Model.VMiti = TxtMiti.Text;
            _objPurchaseQuotation.Model.LedgerId = Convert.ToInt32(TxtVendor.Tag.ToString());

            _objPurchaseQuotation.Model.SalesmanId = Convert.ToInt32(TxtSalesman.Tag.ToString());

            if (!string.IsNullOrEmpty(TxtDepartment.Text))
            {
                string[] dept = TxtDepartment.Tag.ToString().Split('|');
                _objPurchaseQuotation.Model.DepartmentId1 = ((dept[0].ToString() == "") ? 0 : Convert.ToInt32(dept[0].ToString()));
                _objPurchaseQuotation.Model.DepartmentId2 = ((dept[1].ToString() == "") ? 0 : Convert.ToInt32(dept[1].ToString()));
                _objPurchaseQuotation.Model.DepartmentId3 = ((dept[2].ToString() == "") ? 0 : Convert.ToInt32(dept[2].ToString()));
                _objPurchaseQuotation.Model.DepartmentId4 = ((dept[3].ToString() == "") ? 0 : Convert.ToInt32(dept[3].ToString()));
            }
            _objPurchaseQuotation.Model.BranchId = ClsGlobal.BranchId;
            _objPurchaseQuotation.Model.CompanyUnitId = ClsGlobal.CompanyUnitId;
            if (!string.IsNullOrEmpty(TxtCurrency.Tag.ToString()))
            {

                _objPurchaseQuotation.Model.CurrencyId = Convert.ToInt32(TxtCurrency.Tag.ToString());
            }
            _objPurchaseQuotation.Model.CurrencyRate = _CurrencyRate;
            decimal.TryParse(LblBasicAmt.Text.Trim(), out decimal BasicAmt);
            _objPurchaseQuotation.Model.BasicAmount = BasicAmt;
            decimal.TryParse(TxtBillTermAmt.Text.Trim(), out decimal TermAmt);
            _objPurchaseQuotation.Model.TermAmount = TermAmt;
            decimal.TryParse(LblNetAmt.Text.Trim(), out decimal NetAmt);
            _objPurchaseQuotation.Model.NetAmount = NetAmt;
            decimal.TryParse(LblNetAmt.Text.Trim(), out decimal localNetAmount);
            _objPurchaseQuotation.Model.LocalNetAmount = localNetAmount;
            _objPurchaseQuotation.Model.SubLedgerId = Convert.ToInt32(TxtSubledger.Tag.ToString());
            _objPurchaseQuotation.Model.IndentNo = TxtIndentNo.Text.ToString();

			_objPurchaseQuotation.Model.PartyName = !string.IsNullOrEmpty(CashPartyViewModel.PartyName) ? CashPartyViewModel.PartyName : TxtVendor.Text.Trim();
			_objPurchaseQuotation.Model.PartyVatNo = CashPartyViewModel.PartyVatNo;
			_objPurchaseQuotation.Model.PartyAddress = CashPartyViewModel.PartyAddress;
			_objPurchaseQuotation.Model.PartyMobileNo = CashPartyViewModel.PartyMobileNo;
			//_objPurchaseChallan.Model.PartyEmail = CashPartyViewModel.PartyEmail;
			if (!string.IsNullOrEmpty(CashPartyViewModel.ChequeNo))
			{
				_objPurchaseQuotation.Model.ChequeNo = CashPartyViewModel.ChequeNo;
				_objPurchaseQuotation.Model.ChequeDate = Convert.ToDateTime(CashPartyViewModel.ChequeDate);
				_objPurchaseQuotation.Model.ChequeMiti = null;
			}
			else
			{
				_objPurchaseQuotation.Model.ChequeNo = null;
				_objPurchaseQuotation.Model.ChequeDate = null;
				_objPurchaseQuotation.Model.ChequeMiti = null;
			}
            ...
            {
                _objPurchaseQuotation.ModelBillAddress.LedgerId = Convert.ToInt32(TxtVendor.Tag.ToString());
                _objPurchaseQuotation.ModelBillAddress.BillingAddress = TxtBillingAddress.Text.Trim();
                _objPurchaseQuotation.ModelBillAddress.BillingCity = TxtBillingCity.Text.Trim();
                _objPurchaseQuotation.ModelBillAddress.BillingState = TxtBillingState.Text.Trim();
                _objPurchaseQuotation.ModelBillAddress.BillingCountry = TxtBillingCountry.Text.Trim();
                _objPurchaseQuotation.ModelBillAddress.BillingEmail = TxtBillingEmail.Text.Trim();
                _objPurchaseQuotation.ModelBillAddress.ShippingAddress = TxtShippingAddress.Text.Trim();
                _objPurchaseQuotation.ModelBillAddress.ShippingCity = TxtShippingCity.Text.Trim();
                _objPurchaseQuotation.ModelBillAddress.ShippingState = TxtShippingState.Text.Trim();
                _objPurchaseQuotation.ModelBillAddress.ShippingCountry = TxtShippingCountry.Text.Trim();
                _objPurchaseQuotation.ModelBillAddress.ShippingEmail = TxtShippingEmail.Text.Trim();
            }

            _objPurchaseQuotation.ModelOtherDetails.Transport = TxtTransport.Text.Trim();
            _objPurchaseQuotation.ModelOtherDetails.VehicleNo = TxtVehicleNo.Text.Trim();
            _objPurchaseQuotation.ModelOtherDetails.Package = TxtPackage.Text.Trim();
            _objPurchaseQuotation.ModelOtherDetails.CnNo = TxtCnNo.Text.Trim();

            if (TxtCnDate.Text != "  /  /")
            {
                if (ClsGlobal.DateType == "M")
                    _objPurchaseQuotation.ModelOtherDetails.CnDate = _objDate.GetDate(TxtCnDate.Text);
                else
                    _objPurchaseQuotation.ModelOtherDetails.CnDate = Convert.ToDateTime(TxtCnDate.Text);
            }
            else
                _objPurchaseQuotation.ModelOtherDetails.CnDate = null;

            _objPurchaseQuotation.ModelOtherDetails.CnFreight = TxtCnFreight.Text.Trim();
            _objPurchaseQuotation.ModelOtherDetails.Advance = TxtAdvance.Text.Trim();
            _objPurchaseQuotation.ModelOtherDetails.BalFreight = TxtBalFreight.Text.Trim();

            if (CmbCnType.Text == "Due")
            {
                _objPurchaseQuotation.ModelOtherDetails.CnType = "D";
            }
            else if (CmbCnType.Text == "To Pay")
            {
                _objPurchaseQuotation.ModelOtherDetails.CnType = "T";
            }
            else if (CmbCnType.Text == "Paid")
            {
                _objPurchaseQuotation.ModelOtherDetails.CnType = "P";
            }
            else if (CmbCnType.Text == "Free")
            {
                _objPurchaseQuotation.ModelOtherDetails.CnType = "F";
            }
            else if (CmbCnType.Text == "")
            {
                _objPurchaseQuotation.ModelOtherDetails.CnType = null;
            }
            _objPurchaseQuotation.ModelOtherDetails.DriverName = TxtDriverName.Text.Trim();
            _objPurchaseQuotation.ModelOtherDetails.DriverLicNo = TxtDriverLicenseNo.Text.Trim();
            _objPurchaseQuotation.ModelOtherDetails.DriverMobileNo = TxtDriverMobileNo.Text.Trim();
            _objPurchaseQuotation.ModelOtherDetails.ContractNo = TxtContactNo.Text.Trim();
            if (TxtContactDate.Text != "  /  /")
            {
                if (ClsGlobal.DateType == "M")
                    _objPurchaseQuotation.ModelOtherDetails.ContractDate = _objDate.GetDate(TxtContactDate.Text);
                else
                    _objPurchaseQuotation.ModelOtherDetails.ContractDate = Convert.ToDateTime(TxtContactDate.Text);
            }
            else
                _objPurchaseQuotation.ModelOtherDetails.ContractDate = null;

            _objPurchaseQuotation.ModelOtherDetails.ExpInvNo = TxtExportInvNo.Text.Trim();

            if (TxtExportInvDate.Text != "  /  /")
            {
                if (ClsGlobal.DateType == "M")
                    _objPurchaseQuotation.ModelOtherDetails.ExpInvDate = _objDate.GetDate(TxtExportInvDate.Text);
                else
                    _objPurchaseQuotation.ModelOtherDetails.ExpInvDate = Convert.ToDateTime(TxtExportInvDate.Text);
            }
            else
                _objPurchaseQuotation.ModelOtherDetails.ExpInvDate = null;

            _objPurchaseQuotation.ModelOtherDetails.PoNo = TxtPONo.Text.Trim();

            if (TxtPODate.Text != "  /  /")
            {
                if (ClsGlobal.DateType == "M")
                    _objPurchaseQuotation.ModelOtherDetails.PoDate = _objDate.GetDate(TxtPODate.Text);
                else
                    _objPurchaseQuotation.ModelOtherDetails.PoDate = Convert.ToDateTime(TxtPODate.Text);
            }
            else
                _objPurchaseQuotation.ModelOtherDetails.PoDate = null;

            _objPurchaseQuotation.ModelOtherDetails.DocBank = TxtDocBank.Text.Trim();
            _objPurchaseQuotation.ModelOtherDetails.LcNo = TxtLCNo.Text.Trim();
            _objPurchaseQuotation.ModelOtherDetails.CustomName = TxtCustomerName.Text.Trim();
            _objPurchaseQuotation.ModelOtherDetails.Cofd = TxtCoFd.Text.Trim();

            PurchaseQuotationDetailsViewModel _PurchaseQuotationDetails = null;
            TermViewModel _PurchaseQuotationTerm = null;
            ...
            foreach (DataGridViewRow ro in Grid.Rows)
            {
                ...
                {
					_PurchaseQuotationDetails = new PurchaseQuotationDetailsViewModel();
					_PurchaseQuotationDetails.VoucherNo = _objPurchaseQuotation.Model.VoucherNo;
					_PurchaseQuotationDetails.Sno = Grid.Rows.IndexOf(ro) + 1;
					_PurchaseQuotationDetails.ProductId = Convert.ToInt32(ro.Cells["ProductId"].Value.ToString());

                    if (ro.Cells["GodownId"].Value != null)
						_PurchaseQuotationDetails.GodownId = string.IsNullOrEmpty(ro.Cells["GodownId"].Value.ToString()) ? 0 : Convert.ToInt32(ro.Cells["GodownId"].Value.ToString());
                    else
						_PurchaseQuotationDetails.GodownId = 0;

                    if (ro.Cells["AltQty"].Value != null)
                    {
                        decimal.TryParse(ro.Cells["AltQty"].Value.ToString().Trim(), out decimal _AltQty);
						_PurchaseQuotationDetails.AltQty = _AltQty;
                    }
                    else
						_PurchaseQuotationDetails.AltQty = 0;
                    if (ro.Cells["AltQty"].Value != null)
                    {
						_PurchaseQuotationDetails.ProductAltUnitId = !string.IsNullOrEmpty(ro.Cells["ProductAltUnitId"].Value.ToString()) ? Convert.ToInt32(ro.Cells["ProductAltUnitId"].Value.ToString()) : 0;
                    }

					_PurchaseQuotationDetails.Qty = ro.Cells["Qty"].Value != null ? Convert.ToDecimal(ro.Cells["Qty"].Value.ToString().Trim()) : 0;

                    if (ro.Cells["ProductUnitId"].Value != null)
						_PurchaseQuotationDetails.ProductUnitId = string.IsNullOrEmpty(ro.Cells["ProductUnitId"].Value.ToString()) ? 0 : Convert.ToInt32(ro.Cells["ProductUnitId"].Value.ToString());
                    else
						_PurchaseQuotationDetails.ProductUnitId = 0;


					_PurchaseQuotationDetails.ConversionRatio = ro.Cells["QtyConversion"].Value != null ? Convert.ToDecimal(ro.Cells["QtyConversion"].Value.ToString().Trim()) : 0;
					_PurchaseQuotationDetails.PurchaseRate = ro.Cells["Rate"].Value != null ? Convert.ToDecimal(ro.Cells["Rate"].Value.ToString().Trim()) : 0;
					_PurchaseQuotationDetails.BasicAmount = ro.Cells["BasicAmt"].Value != null ? Convert.ToDecimal(ro.Cells["BasicAmt"].Value.ToString().Trim()) : 0;
					_PurchaseQuotationDetails.TermAmount = ro.Cells["TermsAmt"].Value != null ? Convert.ToDecimal(ro.Cells["TermsAmt"].Value.ToString().Trim()) : 0;
					_PurchaseQuotationDetails.NetAmount = ro.Cells["NetAmt"].Value != null ? Convert.ToDecimal(ro.Cells["NetAmt"].Value.ToString().Trim()) : 0;
					_PurchaseQuotationDetails.AdditionalDesc = ro.Cells["AdditionalDesc"].Value != null ? ro.Cells["AdditionalDesc"].Value.ToString() : ro.Cells["Particular"].Value.ToString().Trim();
					_PurchaseQuotationDetails.FreeQty = 0;
					if (ro.Cells["IndentSNo"].Value != null)
						_PurchaseQuotationDetails.IndentSNo = ro.Cells["IndentSNo"].Value != null ? Convert.ToInt32(ro.Cells["IndentSNo"].Value.ToString().Trim()) : 0;
					else
						_PurchaseQuotationDetails.IndentSNo = 0;

					_PurchaseQuotationDetails.IndentNo = ro.Cells["IndentNo"].Value?.ToString().Trim();

					_objPurchaseQuotation.ModelDetails.Add(_PurchaseQuotationDetails);
                    ...
                    if (ro.Cells["TermDetails"].Value != null)
                    {
                        if (!string.IsNullOrEmpty(ro.Cells["TermDetails"].Value.ToString()))
                        {
                            ...
                            {
								_PurchaseQuotationTerm = new TermViewModel();
								_PurchaseQuotationTerm.VoucherNo = TxtVoucherNo.Text;
								_PurchaseQuotationTerm.TermId = Convert.ToInt32(termCodeArr[i]);
								_PurchaseQuotationTerm.Sno = Grid.Rows.IndexOf(ro) + 1;
								_PurchaseQuotationTerm.ProductId = Convert.ToInt32(ro.Cells["ProductId"].Value.ToString());
								_PurchaseQuotationTerm.PTSign = termSignArr[i];
								_PurchaseQuotationTerm.TermType = "P";
								_PurchaseQuotationTerm.TermRate = Convert.ToDecimal(termRatePercentArr[i]);
								_PurchaseQuotationTerm.TermAmt = Convert.ToDecimal(termAmountntArr[i]);
                                _objPurchaseQuotation.ModelTerms.Add(_PurchaseQuotationTerm);
                            }
                        }
                    }
                }
                ...
                if (!string.IsNullOrEmpty(TxtBillTermAmt.Tag.ToString().Trim()))
                {
                    ...
                    if (val2.Length > 1)
                    {
                        ...
                        {
						_PurchaseQuotationTerm = new TermViewModel();
						_PurchaseQuotationTerm.VoucherNo = _objPurchaseQuotation.Model.VoucherNo;
						_PurchaseQuotationTerm.TermId = Convert.ToInt32(termCodeArr1[i]);
						_PurchaseQuotationTerm.Sno = 0;
						_PurchaseQuotationTerm.ProductId = 0;
						_PurchaseQuotationTerm.TermType = "B";
						...
						_PurchaseQuotationTerm.TermRate = _termRatePercentArr1;
						_PurchaseQuotationTerm.TermAmt = _termAmountArr1;
						_PurchaseQuotationTerm.PTSign = termSignArr[i];
                            _objPurchaseQuotation.ModelTerms.Add(_PurchaseQuotationTerm);
                        }
                    }
            ...
            _objPurchaseQuotation.Model.DocId = Convert.ToInt32(TxtVoucherNo.Tag.ToString());
            string output = _objPurchaseQuotation.SavePurchaseQuotation();
        ...
        private void FrmPurchaseQuotation_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void TxtVoucherNo_Validating(object sender, CancelEventArgs e)
        {
            ...
            if (_Tag == "NEW")
            {
                if (!string.IsNullOrEmpty(_objPurchaseQuotation.IsExistsVNumber(TxtVoucherNo.Text)))
                {
			...
			else if (_Tag == "EDIT" || _Tag == "DELETE")
			{
				if (string.IsNullOrEmpty(_objPurchaseQuotation.IsExistsVNumber(TxtVoucherNo.Text)))
				{
					MessageBox.Show("Purchase quotation number does not exist.", "Mr. Solution", MessageBoxButtons.OK, MessageBoxIcon.Information);
					e.Cancel = true;
					return;
				}
				else
				{
					...
					if (this._VoucherNo != TxtVoucherNo.Text.Trim())
					{
						...
						ds = _objPurchaseQuotation.GetDataPurchaseQuotationVoucher(this._VoucherNo);
						ClearFld();
						SetData(ds);
					}

					string _vno1 = _objPurchaseQuotation.IsQuotationUsedInOrder(TxtVoucherNo.Text);
					if (!string.IsNullOrEmpty(_vno1))
					{
						MessageBox.Show("This quotation is used in purchase order.", "Mr. Solution", MessageBoxButtons.OK, MessageBoxIcon.Information);
						_VoucherNo = _vno1; _isSalesQuotationUsed = true; BtnOk.Enabled = false;
						return;
					}

					string _vno2 = _objPurchaseQuotation.IsQuotationUsedInChallan(TxtVoucherNo.Text);
					if (!string.IsNullOrEmpty(_vno2))
					{
						MessageBox.Show("This quotation is used in purchase challan.", "Mr. Solution", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void BtnVoucherNoSearch_Click(object sender, EventArgs e)
        {
            ...
            PickList frmPickList = new PickList("PurchaseQuotation", _SearchKey);
            if (PickList.dt.Rows.Count > 0)
            {
                ...
                if (frmPickList.SelectedList.Count > 0 && _Tag != "NEW")
                {
                    ...
                    DataSet ds = _objPurchaseQuotation.GetDataPurchaseQuotationVoucher(_VoucherNo);
                    ClearFld();
                    SetData(ds);
                }
                frmPickList.Dispose();
		...
		private void SetGridValueToTextBox(int row)
        {
            ...
            if (Grid["Particular", row].Value != null)
            {
                ...
                TxtGridTermsDetails.Text = string.IsNullOrEmpty(Grid["TermDetails", row].Value.ToString())?"": Grid["TermDetails", row].Value.ToString();
            }
            else
            {
                LblShortName.Text = "";
