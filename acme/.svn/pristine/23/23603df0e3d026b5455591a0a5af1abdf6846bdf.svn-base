...
{
    public partial class FrmCreditNoteVoucher : Form
    {
        ...
        ICreditNoteVoucher _objCreditNoteVoucher = new ClsCreditNoteVoucher();
        char DocType = 'B';
        char _IsVatReg = 'N';
        string _VoucherNo = "";
        string _Tag = "";
        ...
        private void FrmCreditNoteVoucher_Load(object sender, EventArgs e)
        {
        ...
        private void FrmCreditNoteVoucher_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void BtnNew_Click(object sender, EventArgs e)
        {
            ...
            this.Text = "Credit Note Voucher [NEW]";
			...
			if (TxtDate.Enabled == false)
				Utility.GetVoucherNo2("Credit Note", TxtVoucherNo, TxtReferenceNo, _Tag, "", _DocId);
			else
				Utility.GetVoucherNo1("Credit Note", TxtVoucherNo, TxtMiti, _Tag, "", _DocId);
        ...
        private void BtnDelete_Click(object sender, EventArgs e)
        {
            ...
            Text = "Credit Note Voucher [DELETE]";
        ...
        private void BtnOk_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Credit note voucher details not found.", "Mr Solution", MessageBoxButtons.OK, MessageBoxIcon.Information);
                Grid.Focus();
                GridControlMode(true);
                return;
            }

            _objCreditNoteVoucher.Model.Tag = _Tag;
			...
			if (_Tag == "NEW")
			{
				string[] VoucherNoDetails = _objCommon.GetVoucherNo(TxtVoucherNo.Tag.ToString(), "Credit Note", ClsGlobal.BranchId, ClsGlobal.CompanyUnitId);
				_objCreditNoteVoucher.Model.DocId = ((TxtVoucherNo.Tag.ToString() == "") ? 0 : Convert.ToInt32(TxtVoucherNo.Tag.ToString()));
				_objCreditNoteVoucher.Model.VoucherNo = VoucherNoDetails[0];
			}
			else
				_objCreditNoteVoucher.Model.VoucherNo = TxtVoucherNo.Text.Trim();

			_objCreditNoteVoucher.Model.VDate = Convert.ToDateTime(TxtDate.Text.ToString());

            _objCreditNoteVoucher.Model.VTime = Convert.ToDateTime(TxtDate.Text.ToString());
            _objCreditNoteVoucher.Model.VMiti = TxtMiti.Text;
            _objCreditNoteVoucher.Model.CurrencyId = ((TxtCurrency.Tag.ToString() == "") ? 0 : Convert.ToInt32(TxtCurrency.Tag.ToString()));
            _objCreditNoteVoucher.Model.CurrencyRate = ((TxtCurrencyRate.Text.ToString() == "" || Convert.ToDecimal(TxtCurrencyRate.Text.ToString()) == 0) ? 1 : Convert.ToDecimal(TxtCurrencyRate.Text.ToString()));
            _objCreditNoteVoucher.Model.LedgerId = ((TxtPartyName.Tag.ToString() == "") ? 0 : Convert.ToInt32(TxtPartyName.Tag.ToString()));
            _objCreditNoteVoucher.Model.SalesmanId = ((TxtSalesman.Tag.ToString() == "") ? 0 : Convert.ToInt32(TxtSalesman.Tag.ToString()));
            _objCreditNoteVoucher.Model.SubLedgerId = ((TxtSubledger.Tag.ToString() == "") ? 0 : Convert.ToInt32(TxtSubledger.Tag.ToString()));

            if (!string.IsNullOrEmpty(this.TxtDepartment.Text))
            {
                string[] dept = TxtDepartment.Tag.ToString().Split('|');
                _objCreditNoteVoucher.Model.DepartmentId1 = ((dept[0].ToString() == "") ? 0 : Convert.ToInt32(dept[0].ToString()));
                _objCreditNoteVoucher.Model.DepartmentId2 = ((dept[1].ToString() == "") ? 0 : Convert.ToInt32(dept[1].ToString()));
                _objCreditNoteVoucher.Model.DepartmentId3 = ((dept[2].ToString() == "") ? 0 : Convert.ToInt32(dept[2].ToString()));
                _objCreditNoteVoucher.Model.DepartmentId4 = ((dept[3].ToString() == "") ? 0 : Convert.ToInt32(dept[3].ToString()));
            }

            _objCreditNoteVoucher.Model.BranchId = ClsGlobal.BranchId;
            _objCreditNoteVoucher.Model.CompanyUnitId = ClsGlobal.CompanyUnitId;
            _objCreditNoteVoucher.Model.ReferenceNo = TxtReferenceNo.Text;
            if (TxtReferenceDate.Text == "  /  /")
                _objCreditNoteVoucher.Model.ReferenceDate = null;
            else
                _objCreditNoteVoucher.Model.ReferenceDate = Convert.ToDateTime(TxtReferenceDate.Text.ToString());
            _objCreditNoteVoucher.Model.EnterBy = ClsGlobal.LoginUserCode;
            _objCreditNoteVoucher.Model.Remarks = TxtRemarks.Text;
            _objCreditNoteVoucher.Model.IsReconcile = 0;
            _objCreditNoteVoucher.Model.ReconcileBy = "";
            _objCreditNoteVoucher.Model.ReconcileDate = null;
            _objCreditNoteVoucher.Model.IsPosted = 0;
            _objCreditNoteVoucher.Model.PostedBy = "";
            _objCreditNoteVoucher.Model.PostedDate = null;
            _objCreditNoteVoucher.Model.IsAuthorized = 0;
            _objCreditNoteVoucher.Model.AuthorizedBy = "";
            _objCreditNoteVoucher.Model.AuthorizedDate = null;
            _objCreditNoteVoucher.Model.AuthorizeRemarks = "";
            _objCreditNoteVoucher.Model.Gadget = "Desktop";

            CreditNoteDetailsViewModel DetailsModel = null;
            foreach (DataGridViewRow ro in Grid.Rows)
            {
                if (ro.Cells["GeneralLedger"].Value != null)
                {
                    DetailsModel = new CreditNoteDetailsViewModel();
                    DetailsModel.VoucherNo = _objCreditNoteVoucher.Model.VoucherNo;
                    ...
                    if (ro.Cells["Amount"].Value != null)
                    {
                        ...
                        DetailsModel.LocalAmount = (ro.Cells["Amount"].Value.ToString() != "" ? Convert.ToDecimal(ro.Cells["Amount"].Value.ToString()) : 0) * _objCreditNoteVoucher.Model.CurrencyRate;
                    }
                    else
                    {
                    ...
                    _objCreditNoteVoucher.ModelDetails.Add(DetailsModel);
                }
            }
            ...
            if (_Tag == "NEW")
            {
                if (ClsGlobal.ConfirmSave == 1)
                {
                    ...
                    if (dialogResult == DialogResult.Yes)
                        result = _objCreditNoteVoucher.SaveCreditNote();
                    else
                        return;
                }
                else
        ...
        private void TxtVoucherNo_Validating(object sender, CancelEventArgs e)
        {
            ...
            if (_Tag == "NEW")
            {
                if (!string.IsNullOrEmpty(_objCreditNoteVoucher.IsExistsVNumber(TxtVoucherNo.Text)))
                {
			...
			else
			{
				if (this._VoucherNo != TxtVoucherNo.Text.Trim())
				{
					...
					DataSet ds = _objCreditNoteVoucher.GetDataCreditNoteVoucher(this._VoucherNo);
					ClearFld();
					SetData(ds);
				}
			}
        ...
        private void BtnVoucherNoSearch_Click(object sender, EventArgs e)
        {
            ...
            PickList frmPickList = new PickList("CreditNoteVoucher", _SearchKey);
